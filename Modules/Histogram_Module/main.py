...
class HistogramPlotter:
    def __init__(self, lower_limit=None, upper_limit=None):
        ...
        self.fig, self.ax = plt.subplots(figsize=(8, 4.8), facecolor='black')
    ...
    def plot_horizontal_histogram_with_bell_curve(self, data):
        ...
        if self.lower_limit is not None and self.upper_limit is not None:
            n, bins, patches = self.ax.hist(data, bins=22, range=(self.lower_limit, self.upper_limit), orientation='horizontal', alpha=0.7, color='#5DADE2', edgecolor='white', label='Histogram')
        else:
            n, bins, patches = self.ax.hist(data, bins=22, orientation='horizontal', alpha=0.7, color='#5DADE2', edgecolor='white', label='Histogram')
