...
class Keyboard(QtWidgets.QWidget):
    ...
    def __init__(self,target_lineedit=None,parent=None):
        try:
            ...
            self.headerLabel.setStyleSheet("color: white; font-size: 15px; border: none;")
            ...
            self.input_box.setStyleSheet('font: 15pt "MS Shell Dlg 2"; color: white; background-color: black;')
    ...
    def handle_close(self):
        try:
            # Close keyboard safely and notify listeners
            self.keyboardClosed.emit()
            self.close()
        except Exception as e:
            QtWidgets.QMessageBox.critical(self, "Close Error", str(e))
    ...
    def showEvent(self, event):
        try:
            # Position keyboard relative to parent and show safely
            parent = self.parentWidget()
            if parent:
            ...
            QtWidgets.QMessageBox.critical(self, "Show Event Error", str(e))
    ...
    def create_keyboard(self):
        try:
            # Rebuild keyboard buttons with current case/symbol state
        ...
        try:
            ...
            def mk_btn(text, w):
                try:
                    ...
                    QtWidgets.QMessageBox.critical(self, "Button Create Error", str(e))
            ...
            self.btn_symbols = mk_btn("123", 60)
    ...
    def toggle_case(self):
        try:
            # If currently in number/symbol mode,
            # pressing ab/AB should return to alphabet mode.
            if self.show_symbols:
                self.show_symbols = False
                self.create_keyboard()
                return

            # Otherwise just change upper/lower case.
            self.is_upper = not self.is_upper

            for btn in self.buttons:
                if btn.text().isalpha():
                    btn.setText(
                        btn.text().upper() if self.is_upper
                        else btn.text().lower()
                    )
