...
class SPIController(QObject):
    ...
    def __init__(self, main_window, data_handler,app_controller):
        try:
            ...
            self.data.probe_setttings_saved.connect(self.load_least_counts)
    ...
    #         # -------------------------------
    #         # CLEAN OLD STATUS (IMPORTANT 🔥)
    #         # -------------------------------
    #         active_probes = set(self.probe_data.keys())

    #         self.last_status = {
    #             k: v for k, v in self.last_status.items()
    #             if k in active_probes
    #         }

    #         # -------------------------------
    #         # 3️⃣ FINAL LOOP (UI = INSTANT)
    #         # -------------------------------
    #         for probe, pdata in self.probe_data.items():
    ...
    #             # -------------------------------
    #             # STABILITY CHECK (ONLY FOR RELAY)
    #             # -------------------------------
    ...
    #         if not hasattr(self, "previous_final_values"):
    #             self.previous_final_values = {}
    ...
    #             curr = calculated_values.get(probe)
    #             prev = self.previous_final_values.get(probe)

    #             if curr is None:
    #                 continue

    #             # compare rounded/final values
    #             lc = self.get_lc_cached(probe)

    #             curr = self.smash(curr, lc)

    #             if prev != curr:
    #                 value_changed = True

    #             self.previous_final_values[probe] = curr
    #         # Ignore dimension add/remove
    #         if set(self.previous_status.keys()) != set(self.last_status.keys()):
    #             status_changed = False
    #         else:
    #             status_changed = (self.previous_status != self.last_status)
    ...
    #             if value_changed and self.relay_triggered_for_cycle:
    ...
    #             # WAIT STABLE TIME
    #             if (
    #                 not self.relay_triggered_for_cycle and
    #                 (current_time - self.last_trigger_time) >= self.auto_save_duration
    #             ):
