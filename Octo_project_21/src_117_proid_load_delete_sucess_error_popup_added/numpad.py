...
class Ui_Numpad(object):
    def setupUi(self, Numpad, on_input=None):
        try:
            ...
            self.Numpad.resize(300, 450)
            self.Numpad.setMinimumSize(QtCore.QSize(300, 450))
            self.Numpad.setMaximumSize(QtCore.QSize(300, 450))
            
            # self.Numpad.setObjectName("mainWidget")

            self.Numpad.setStyleSheet("""
                #Numpad{
                    background-color: black;
                    border: 2px solid white;
                }
            """)  
            # self.Numpad.setStyleSheet("background-color: #2b2b2b;border: 1px solid grey;")  # dark grey background
            ...
            self.Input_box.setStyleSheet("color: white; background-color: black; padding: 5px;")
...
class numpad_window(Ui_Numpad,QtWidgets.QWidget):
    ...
    def __init__(self, target_lineedit=None, parent=None):
        try:
            ...
            self.buttons['OK'].setStyleSheet("""
                QPushButton {
                    color: white;
                    background-color: #0f401b;   /* Normal green */
                    font: 14pt 'MS Shell Dlg 2';
                    border: 1px solid #777777;
                    border-radius: 6px;
                }
                ...
                QPushButton:hover {
                    background-color: #19692c;   /* Slightly lighter green */
                }

                QPushButton:pressed {
                    background-color: #19692c;   /* Slightly darker green */
                }
            """)
            # self.buttons['OK'].setStyleSheet(("""
            #     QPushButton {
        ...
        try:
            if self.target_lineedit:
                self.target_lineedit.setText(self.Input_box.text())
                            # ✅ SAFE CHECK
                parent = self.target_lineedit.parentWidget()

                if parent is not None:
                    parent.setFocus()
            # ❌ REMOVE THIS LINE
            # self.Input_box.clear()
            # ✅ CALL EXTRA CALLBACK IF EXISTS
...
class TouchOverlay(QtWidgets.QWidget):
