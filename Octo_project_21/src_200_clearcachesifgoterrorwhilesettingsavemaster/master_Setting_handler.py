...
class SetMasterController(QObject):
    ...
    #             # Converted factor is per-probe, shared across every ProgramId.
    #             conversion_rows = session.query(SingleMasterCalibration).all()

    #             for calibration in conversion_rows:
    #                 if calibration.ConvertedRawValue is not None:
    #                     probe = self.normalize_calibration_probe(calibration.Probe)
    #                     self.single_master_converted_raw_value.setdefault(
    #                         probe, calibration.ConvertedRawValue
    #                     )

    #             # 🔥 CHANGED: LOW/HIGH factory calibration is also per-probe hardware
    #             # data, not per-program — so stop filtering by ProgramId. Instead,
    #             # pull every row and keep the first fully-populated one seen per
    #             # probe/dimension, regardless of which ProgramId it was saved under.
    #             rows = session.query(SetMasterSettings).all()

    #             if not rows:
    #                 print(
    #                     "[SINGLE MASTER] No saved factory calibration found "
    #                     "for any ProgramId"
    #                 )
    #                 return False
    ...
    #                 # Don't overwrite a probe we already picked up from an
    #                 # earlier (e.g. more specific / earlier) row.
    #                 if probe in self.single_master_low_spi and probe in self.single_master_high_spi:
    #                     continue
    ...
    #                     f"(from ProgramId={row.ProgramId})"
    #                 )

    #             if not loaded_any:
    #                 print(
    #                     f"[SINGLE MASTER] No usable factory calibration rows "
    #                     f"found (checked {len(rows)} rows across all ProgramIds)"
    #                 )
    #                 return False
    ...
    #             # -----------------------------------
    #             # GET ALREADY LOADED LOW/HIGH VALUES
    #             # These were loaded from SetMasterSettings
    #             # in load_single_master_calibration_db()
    #             # -----------------------------------
    #             spi_low = self.single_master_low_spi.get(
    #                 calibration_dimension
    #             )
    
    #             spi_high = self.single_master_high_spi.get(
    #                 calibration_dimension
    #             )
    
    #             # Fallback only for P/D key mismatch
    #             if spi_low is None:
    ...
    #             DB_low = self.single_master_low_value.get(
    #                 calibration_dimension
    #             )
    
    #             DB_high = self.single_master_high_value.get(
    #                 calibration_dimension
    #             )
    ...
    #             # -----------------------------------
    #             # CHECK SAVED CONVERSION
    #             # SingleMasterCalibration
    #             # -----------------------------------
    #             active_program_id = int(
    #                 self.ui_parent.valueObj.activeVariables_dict[
    #                     "ActiveProgramId"
    #                 ]
    #             )
    ...
    #             # Fallback to dimension/probe if required
    ...
    #             # -----------------------------------
    #             # IF NOT SAVED:
    #             # CALCULATE USING ALREADY LOADED VALUES
    #             # -----------------------------------
    #             if converted_raw_value is None:
    ...
    #                 converted_raw_value = (
    #                     (DB_high - DB_low) / raw_range
    #                 )
    
    #                 # Inch conversion is required only
    #                 # for a newly calculated factor
    ...
    #                 # -----------------------------------
    #                 # SAVE ONLY CALCULATED CONVERSION
    #                 # INTO SingleMasterCalibration
    #                 # -----------------------------------
    #                 self.save_single_master_runtime_calibration(
    #                     row,
    #                     converted_raw_value
    #                 )
    ...
    def Nomaster_label(self, calculated_values, current_time=None):
        ...
        try:
            ...
            for probe, pdata in no_master_limits.items():
                ...
                # Autosense has priority and resets the stable-status timer,
