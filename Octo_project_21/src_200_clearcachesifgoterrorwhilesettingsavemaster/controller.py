...
class AppController(QObject):
    ...
    def __init__(self, main_window, data_handler):
        try:
            ...
            self.data.probe_setttings_saved.connect(
                self.refresh_program_ids_after_probe_save
            )
            self.data.probe_setttings_saved.connect(
                lambda: self.Handle_master_Grouping(
                    self.safe_int(
    ...
    def refresh_program_ids_after_probe_save(self):
        """Add a newly saved program to selector lists without rebuilding them."""
        try:
            program_id = str(
                self.ui.valueObj.activeVariables_dict.get("ActiveProgramId", 1)
            )
            if self.ui.comboBox_programIdSetting_outer.findText(program_id) == -1:
                self.init_progId_and_dimension()
        except Exception as e:
            print(f"Error refreshing program IDs after probe save: {e}")
    ...
    def handle_grouping_buttons(self):
        try:
            ...
            if "fec822" in btn.styleSheet() and self.ui.masterGroupingChannel_1:
                btn.setStyleSheet("""
                        background-color: black;
                        color: white;
                        font-size: 13pt;
            ...
            self.D1_masterGrouping = "#fec822" in self.ui.masterGroupingChannel_1.styleSheet().lower()
            self.D2_masterGrouping = "#fec822" in self.ui.masterGroupingChannel_2.styleSheet().lower()
            self.D3_masterGrouping = "#fec822" in self.ui.masterGroupingChannel_3.styleSheet().lower()
            self.D4_masterGrouping = "#fec822" in self.ui.masterGroupingChannel_4.styleSheet().lower()
        except Exception as e:
    ...
    def progId_changed_inner(self):
        try:
            ...
            self.update_delete_button_visibility()
            self.set_default_formula_label()
            # self.program_deleted_clear_caches.emit(self.ui.valueObj.activeVariables_dict.get("ActiveProgramId", "1"))
            # self.data.update_aoc_dimension_combobox()
        except Exception as e:
    ...
    def Handle_master_Grouping(self, program_id):
        ...
        - Master type configured for each channel
        """
        try:
            ...
            # 3️⃣ Show buttons only for configured channels.  A No Master
            # dimension remains visible so the operator can see it, but it
            # cannot be selected for a master group.
            for channel, button in  self.master_group_buttons_with_channel.items():
                channel_settings = self.ui.valueObj.ProgramSettings_dict.get(channel, {})
                probe_settings = channel_settings.get("ProbeBasedSettings", {})
                master_type = str(probe_settings.get("MasterType", ""))
                is_no_master = "".join(master_type.split()).lower() == "nomaster"

                button.setVisible(channel in available_channels)
                button.setEnabled(channel in available_channels and not is_no_master)
                ...
                if is_no_master:
                    ...
                    """)
                    setattr(self, f"{channel}_masterGrouping", False)
        except Exception as e:
    ...
    def handle_set_datetime(self):
        """
        Called when button_setDateTime is clicked.
    ...
    def linkage_of_saveButton(self):
        try:
            ...
            if current_index == 6:
                ...
                # The save signal refreshes the UI once after this handler
                # returns.  Reloading here as well blocks the GUI twice.
