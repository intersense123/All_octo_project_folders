...
class Utilities:
    """Miscellaneous helpers used across the UI.

    This module contains mostly hardware/OS side effects:
    - evaluate/validate formula bar input
    - board detection (Raspberry Pi vs i.MX7)
    - static IP + system time + WiFi configuration
    - shutdown/restart
    """

    def __init__(self, main_window):
        """Create helper bound to the MainWindow instance."""
    ...
    def validate_formulabar(self):
        """Validate and (attempt to) evaluate the formula bar expression.

        - Replaces D1..D8 and P1..P8 tokens with numeric placeholders.
        - Uses `exec` to validate syntax (no value is returned).
        - Updates the formula bar background color:
            * green  => execution succeeded
            * red    => empty/invalid formula

        Side effects:
            Mutates `self.ui.lineEdit_formulaBar` stylesheet.
        """
    ...
    def is_raspberry_pi(self) -> bool:
        """Return True if running on Raspberry Pi (via device-tree model file)."""
    ...
    def set_system_time_imx7(self):
        ...
        try:
            ...
            subprocess.run(["date", "-s", datetimestr], check=True)
