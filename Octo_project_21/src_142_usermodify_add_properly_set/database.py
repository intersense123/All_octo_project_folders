...
class DatabaseAgent:
    ...
    def upsert_cnc_list(self, cnc_name: str, ip: str, port: int, selection: str, controller: str,trial_mode : str):
        """
        ...
        with self.SessionLocal() as session:
            try:
                existing = session.query(CNCMaster).filter_by(CNCName=cnc_name).first()
                if existing:
                    ...
                    existing.Trial_mode = trial_mode
                else:
                    new_entry = CNCMaster(
                        CNCName=cnc_name,
                        IPAddress=ip,
