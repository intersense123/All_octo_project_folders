...
class SPCBase:
    ...
    def update_limit_artists(self):
        try:
            ...
            # ---------------------------------------------------------
            # Check whether outer limits are available
            # ---------------------------------------------------------
            has_outer = (
                uol != 0 and
                lol != 0 and
                lol < lsl < lcl < nominal < ucl < usl < uol
            )

            # ---------------------------------------------------------
            # Actual limit values remain unchanged
            #
            # The Y-axis transformation will make every interval
            # visually equal.
            # ---------------------------------------------------------
            if has_outer:

                actual_limits = np.array([
                    lol,
                    lsl,
                    lcl,
                    nominal,
                ...
                # Equal visual positions
                display_positions = np.arange(
                    len(actual_limits),
                    dtype=float
                )
            ...
            # ---------------------------------------------------------
            # Extend both sides slightly for graph padding
            # ---------------------------------------------------------
            first_gap = actual_limits[1] - actual_limits[0]
            last_gap = actual_limits[-1] - actual_limits[-2]
            padding = 0.30
            scale_x = np.concatenate([
                [actual_limits[0] - first_gap * padding],
                actual_limits,
                [actual_limits[-1] + last_gap * padding]
            ])

            scale_y = np.concatenate([
                [display_positions[0] - padding],
                display_positions,
                [display_positions[-1] + padding]
            ])

            # ---------------------------------------------------------
            # Custom Y-axis transformation
            #
            # Actual value -> Equal visual spacing
            #
            # IMPORTANT:
            # Your update_graph() still sends ACTUAL values.
            # Matplotlib automatically transforms them.
            # ---------------------------------------------------------
            def forward(values):
            ...
            # Use ACTUAL VALUES - do not change them
            # ---------------------------------------------------------
            def hline(key, y):
                self.hlines[key].set_ydata([y, y])
                self.hlines[key].set_visible(True)

            # Actual saved limit lines
            hline("usl", usl)
            hline("lsl", lsl)
            hline("nominal", nominal)
            hline("ucl", ucl)
            ...
            self.hlines["uol"].set_ydata([uol, uol])
            self.hlines["uol"].set_visible(has_outer)

            self.hlines["lol"].set_ydata([lol, lol])
            self.hlines["lol"].set_visible(has_outer)

            # ---------------------------------------------------------
            # Helper for coloured spans
            # Again: ACTUAL VALUES remain unchanged
            # ---------------------------------------------------------
            def span(key, y0, y1, visible=True):
                self.hspans[key].set_xy([
                    [0, y0],
                    [1, y0],
                    [1, y1],
                    [0, y1],
            ...
            # Actual values
            span("lsl_lcl", lsl, lcl)
            span("lcl_nom", lcl, nominal)
            span("nom_ucl", nominal, ucl)
            span("ucl_usl", ucl, usl)
            ...
            # Y-axis labels
            #
            # Tick locations are ACTUAL limit values.
            # But custom scale displays them with SAME GAP.
            # ---------------------------------------------------------
            self.ax.set_yticks(actual_limits)
            ...
            print(
                f"Error in update_limit_artists -> {e}"
            )      
