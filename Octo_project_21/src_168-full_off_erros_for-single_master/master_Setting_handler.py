    ...
    def __init__(self,
        ...
        try:
            ...
            self.title_label.setStyleSheet(
                "background:#444444;font-size:17pt;font-weight:bold;"
            )
...
class SetMasterController(QObject):
    ...
    def __init__(self, parent, spi_values,spi_signal,grouped_channels=None,
        ...
        try:
            ...
            self.single_master_conversion_factor = {}
    ...
    def run(self):
        ...
        try:
            ...
            self.single_master_masterSpi.clear()
            ...
            for row in rows:
                ...
                if row.MasterType == "Single Master":
                    ...
                    # ----------------------------------------
                    # VALIDATE SINGLE MASTER LOW/HIGH CALIBRATION
                    # ----------------------------------------
                    # for p in selected_probes:

                    #     spi_low = self.single_master_low_spi.get(p)
                    #     spi_high = self.single_master_high_spi.get(p)

                    #     db_low = self.single_master_low_value.get(p)
                    #     db_high = self.single_master_high_value.get(p)

                    #     # LOW/HIGH calibration data is required
                    #     if (
                    #         spi_low is None or
                    #         spi_high is None or
                    #         db_low is None or
                    ...
                    #             "Factory calibration not available",
                    #             "Close",
                    #             parent=self.ui_parent
                    #         )
                    ...
                    #         self.ui_parent.status_manager.show(
                    #             page_index=28,
                    #             message=f"{p}: Please perform factory calibration first",
                    #             timeout=5000,
                    #             msg_type="error"
                    #         )
    ...
    def calculate_single_master(self, row):
        try:
            ...
            self.probe_data[probe] = {
                ...
                "converted_raw_value": converted_raw_value,
                "single_master_conversion_factor": converted_raw_value,
                "master_ready": True
            }
            self.single_master_conversion_factor[probe] = converted_raw_value
...
class SPIController(QObject):
    ...
    #                 msg_type="success"
    #             )
    #             raw_range = high_spi - low_spi

    #             if raw_range == 0:
    #                 return

    #             conversion_factor = (
    #                 (high_val - low_val) / raw_range
    #             )

    #             # Store in runtime
    #             self.single_master_conversion_factor[d_key] = (
    #                 conversion_factor
    #             )
            ...
            with SessionLocal() as session:
                ...
                for probe, data in self.probe_data.items():
                    row = SetMasterSettings(
                        ...
                        single_master_conversion_factor=data.get(
                            "single_master_conversion_factor"
                        )
                    )
            ...
            with SessionLocal() as session:
                ...
                for row in rows:
                    ...
                    if probe_db.MasterType == "Single Master":
                        db_low = row.DB_low
                        db_high = row.DB_high
                        converted_factor = row.single_master_conversion_factor
                        spi_master = row.SPI_Master
                    # ------------------------------------
                    # ✅ DOUBLE MASTER
                    # ------------------------------------
                    else:
                        ...
                        converted_factor = row.MasterValue
                        spi_master = 0.0
                    ...
                    self.probe_data[row.Dimension] = {
                        ...
                        "SPI_Master": spi_master,
                        "converted_raw_value": converted_factor,
                        "master_ready": True
                    }
                    ...
                    calculated_master_value = (
                        pdata["DB_master"]
                            + (
                                ...
                                * pdata["single_master_conversion_factor"]
                            )
                        )     
                else:   
                    calculated_master_value = (
