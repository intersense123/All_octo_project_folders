...
class CNCManager(QObject):
    ...
    def process_reading_and_write_correction(
        ...
        try:
            ...
            LSL = float(f"{probe.LowerSpecificationLimit:.3f}")
            USL = float(f"{probe.UpperSpecificationLimit:.3f}")
            LCL = float(f"{probe.LowerControlLimit:.3f}")
            UCL = float(f"{probe.UpperControlLimit:.3f}")
            mean = float(f"{probe.NominalValue:.3f}")
                        ...
                        if trial_mode == 'OFF':
                            if controller == "FANUC":
                                ...
                                if skip_offset > skip_counter[dimension]:
                                    ...
                                    signed_value = {'serve': signed_value}.get(todo, 0)
                                    print("******** CNC WRITE ********")
                                    print("signed_value =", signed_value)
                                    print("todo =", todo)
