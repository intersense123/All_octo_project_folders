...
# Recreate the RS232 serial handler from current DB settings
    ...
    def setup_program_table(self):
        ...
        try:         
            ...
            self.table_programList.setColumnWidth(0, 200)
            self.table_programList.setColumnWidth(1, 560)
...
"QHeaderView::section { border: 2px solid #888888; }")
            ...
            # Set row height
            self.table_programList.verticalHeader().setDefaultSectionSize(55)
    ...
    def on_program_selected(self, index):
        try:
            ...
            print(f"Selected Program: ID={program_id}, Name={program_name}")
    ...
    def on_ProgramId_load_clicked(self):
        try:
            if hasattr(self, "selected_program_id"):
                ...
                if program_id is not None:
                    confirm_msg = CustomMessageBox(
                        f"Are you really want to load Program ID {program_id}?",
                        "info",
                        parent=self.ui
                    )
                    confirm_msg.exec_()
