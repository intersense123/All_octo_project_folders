...
class SPIController(QObject):
    ...
    def ok_notok_part_check(self):
        try:
            ...
            for probe, pdata in self.probe_data.items():
                ...
                Final_val = self.smash(calculated_master_value, leastcount)
                
                # Format with leastcount decimal places to preserve trailing zeros
                # Convert leastcount to string avoiding scientific notation
                leastcount_str = f"{float(leastcount):.10f}".rstrip('0')
                decimal_places = len(leastcount_str.split('.')[-1]) if '.' in leastcount_str else 0
                formatted_value = f"{Final_val:.{decimal_places}f}"
                
                # Determine status based on FORMATTED value for consistency
                final_value_for_check = float(formatted_value)
            # -------------------------------------------------
            # 2️⃣ FORMULA
            # -------------------------------------------------
            formula_list = self.formula_Cache
            ...
            for probe, pdata in self.probe_data.items():
                ...
                # Run move_needle in a separate thread (calculate angle from value)
                if probe == "D2":
                    # Calculate angle based on where value falls in range
                    angle = self.calculate_angle(
                        final_value,
                        pdata["LSL"],
                        pdata["USL"],
    ...
    def calculate_angle(self, value, nominal, least_count, dial_range):
        try:
            ...
            divisions = deviation / (least_count)
    ...
    def save_probe_values(self):
        ...
        try:
            ...
            for dim in channels:
                ...
                print("SAVE START", time.time())
                self.ui.spc_manager.update_value(ch, value)
                print("UPDATE_VALUE DONE", time.time())
