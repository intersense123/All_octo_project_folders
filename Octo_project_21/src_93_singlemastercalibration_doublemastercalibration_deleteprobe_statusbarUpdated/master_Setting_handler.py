    ...
    def run(self):
        ...
        try:
            ...
            for row in rows:
                ...
                if row.MasterType == "Single Master":
                    ...
                    self.ui_parent.status_manager.show(
                        page_index=28,
                        message=f"Removed part Successful for {', '.join(selected_probes)}",
                        timeout=3000
                    )
                    ...
                    self.ui_parent.status_manager.show(
                        page_index=28,
                        message=f"Master Set Successful for {', '.join(selected_probes)}",
                        timeout=3000
                    )
                    ...
                    for p in selected_probes:
                        ...
                        low_spi = self.single_master_low_spi.get(p)
                        high_spi = self.single_master_high_spi.get(p)

                        if low_spi is None or high_spi is None:

                            popup_fail = Set_master_ui(
                                "Please calibrate both LOW and HIGH master.",
                                "Close",
                                parent=self.ui_parent
                            )
                            ...
                            self.ui_parent.status_manager.show(
                                page_index=28,
                                message=f"Please calibrate both LOW and HIGH master for {', '.join(selected_probes)}",
                                timeout=3000
                            )
                            return
                    ...
                    # CURRENT ROW VALUES
                    # ----------------------------------------
                    row.MasterLower = self.single_master_low_value.get(probe)
                    row.MasterHigher = self.single_master_high_value.get(probe)
            ...
            if current == '0':
                    ...
                    self.ui_parent.status_manager.show(
                        page_index=28,
                        message=f"Removed part Successful for {', '.join(Active_channels)}",
                        timeout=3000
                    )
                    self.spi_signal.disconnect(remove_parts.update_spi_value)
                    if result == QDialog.Rejected:
                    ...
                    self.ui_parent.status_manager.show(
                        page_index=28,
                        message=f"Lower Master Set Successful for {', '.join(Active_channels)}",
                        timeout=3000
                    )
                    if result == QDialog.Rejected:
                        self.ui_parent.status_manager.show(
                    ...
                    self.ui_parent.status_manager.show(
                        page_index=28,
                        message=f"Higher Master Set Successful for {', '.join(Active_channels)}",
                        timeout=3000
                    )
                    if result == QDialog.Rejected:
                        self.ui_parent.status_manager.show(
                    ...
                    self.ui_parent.status_manager.show(
                        page_index=28,
                        message=f"Removed part Successful for {', '.join(self.grouped_channels)}",
                        timeout=3000
                    )
                    if result == QDialog.Rejected:
                        self.ui_parent.status_manager.show(
                    ...
                    self.ui_parent.status_manager.show(
                        page_index=28,
                        message=f"Lower Master Set Successful for {', '.join(self.grouped_channels)}",
                        timeout=3000
                    )
                    if result == QDialog.Rejected:
                        self.ui_parent.status_manager.show(
                    ...
                    self.ui_parent.status_manager.show(
                        page_index=28,
                        message=f"Higher Master Set Successful for {', '.join(self.grouped_channels)}",
                        timeout=3000
                    )
                    if result == QDialog.Rejected:
                        self.ui_parent.status_manager.show(
...
class SPIController(QObject):
    ...
    def update_auto_save(self):
        try:
            ...
            # auto_save_duration += auto_save_duration * 0.1  # add 10% buffer to ensure save happens after stable
            # milliseconds
    ...
    # 🔥 TO GET PROCESSED SPI VALUES BASED ON FORMULA (USED FOR SET MASTER POPUP)     
    ...
    # 🔥 TO UPDATE RAW SPI LABELS IN SET MASTER POPUP     
    ...
    def set_master(self):
        ...
        try:
            self.ui.status_manager.show(
                    page_index=28,
                    message="Master setup started. Please follow all steps.",
                    timeout=0
                )
            current = (
                self.ui.valueObj.IOSettings_dict
        ...
        try:
            ...
            with SessionLocal() as session:
                ...
                for row in rows:
                    ...
                    # ✅ SINGLE MASTER
                    # ------------------------------------
                    if probe_db.MasterType == "Single Master":
                        db_low = row.DB_low
                        db_high = row.DB_high
                    # ------------------------------------
                    # ✅ DOUBLE MASTER
    ...
    def calculate_angle(self, value, nominal, dial_range):
        try:
            # needle rotation as per formula tickwise
            # Use 30 divisions for 0.3/0.03/0.003 range dials, otherwise use 60 divisions
            max_divisions = 30 if dial_range in [0.3, 0.03, 0.003] else 60

            # Calculate difference between measured value and nominal value
            deviation = value - nominal

            # Convert measurement deviation into dial divisions (ticks)
            divisions = deviation / 0.001 #least_count
            # Limit needle movement within dial boundaries (-30/+30 or -60/+60)
            divisions = max(-max_divisions,
                            min(max_divisions, divisions))

            # Convert dial divisions into needle angle (-90° to +90°)
            angle = (divisions / max_divisions) * 90.0
            
            # Return calculated needle angle
            return angle

        except Exception as e:
            # Print error and return center position
