...
class DataHandler(QObject):
    ...
    def calculate_control_limits(self):
        ...
        try:
            ...
            if not self.ui.lineEdit_lcl.text().strip():
                self.ui.lineEdit_lcl.setText(f"{lcl:.3f}")

            if not self.ui.lineEdit_ucl.text().strip():
                self.ui.lineEdit_ucl.setText(f"{ucl:.3f}")
    ...
    # load cnc list to ui
    # def load_cncList_to_ui(self):
    #     try:
    #         # Ensure dictionary is loaded
    #         if (not hasattr(self.ui.valueObj, "CNCList_dict")
    #                 or not self.ui.valueObj.CNCList_dict):
    #             self.ui.valueObj.CNCList_dict = (
    #                 self.ui.databaseObj.load_data_to_CNCList_dict()
    #             )

    #         cnc_dict = self.ui.valueObj.CNCList_dict or {}

    #         self.ui.lineEdit_cncName.setText(
    #             str(cnc_dict.get("CNCName", ""))
    #         )

    #         self.ui.lineEdit_cncIpAddress.setText(
    #             str(cnc_dict.get("IPAddress", ""))
    #         )

    #         self.ui.lineEdit_cncPortNumber.setText(
    #             str(cnc_dict.get("PortNumber", ""))
    #         )

    #         self.ui.comboBox_cncSelectionType.setCurrentText(
    #             str(cnc_dict.get("CNCSelection", ""))
    #         )

    #         self.ui.comboBox_cncController.setCurrentText(
    #             str(cnc_dict.get("ControllerName", ""))
    #         )

    #     except Exception as e:
    #         print(f"Error loading CNC List to UI: {e}")
    # -------- SAVE --------
    ...
    def save_toDatabase_cncList(self):
        try:
            ...
            self.ui.status_manager.show(
                    page_index=13,
                    message="Cnc Data loaded successfully.",
                    timeout=6000,
                    msg_type="success"
                )
            self.ui.cncListDisplay.load_cnc_data_to_table()
            # self.load_cncList_to_ui()
        except Exception as e:
    ...
    def save_toDatabase_userManagement(self):
        ...
        try:
            ...
            self.ui.status_manager.show(
                page_index=2,
                message=f"{msg_text} Successfully.",
                timeout=6000
            )            
            ...
            self.ui.status_manager.show(
                page_index=2,
                message=f"Failed saving/updating user.",
                timeout=6000
            ) 
