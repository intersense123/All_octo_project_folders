...
class DialIndicator(QObject):
    ...
    def __init__(self, main_window):
        try:
            ...
            self._pm = {
                380: QPixmap(u"/home/torizon/app/src/images/dial-img-yellow-380_new.png"),
...
"""
    ...
    def _test_needle(self):
        print("[TEST] _test_needle fired")
        if hasattr(self.ui, "dial_1_anim"):
            print("[TEST] dial_1_anim exists — calling move_needle(90)")
            self.ui.dial_1_anim.move_needle(90)
        else:
            print("[TEST] dial_1_anim does NOT exist yet")  
                    ...
                    if self.ui.button_mode1.text() == 'Dial':
                        ...
                        if not hasattr(self.ui, "dial_1_anim"):
                            self.ui.dial_1_anim = DialHandler(
                                ...
                                needle_pos_y  = 73,    # 182 - 110
                                pivot_point_x = 11,    # SVG width 23 / 2
                                pivot_point_y = 110,   # dot near bottom of 120px SVG
                            )
                        # self.ui.dial_1_anim.start_demo(2000)
                        # self.ui.dial_1_anim.move_needle(90)
                        # self.ui.dial_1_anim.demo_timer.start(500)
                        # if not self.ui.dial_1_anim.demo_timer.isActive():
                        #     self.ui.dial_1_anim.demo_timer.start(1000)
                        # 3️⃣ ALWAYS call show_on_top — raises view above all siblings
                        #    (must be AFTER setGeometry above)
                        # self.ui.dial_1_anim.show_on_top()
                
                        # # 4️⃣ Spring test — comment these out once working with real data
                        # QTimer.singleShot(300,  lambda: self.ui.dial_1_anim.move_needle(0))
                        # QTimer.singleShot(1000, lambda: self.ui.dial_1_anim.move_needle(90))
                        # QTimer.singleShot(2500, lambda: self.ui.dial_1_anim.move_needle(-90))
                        # QTimer.singleShot(4000, lambda: self.ui.dial_1_anim.move_needle(0))
