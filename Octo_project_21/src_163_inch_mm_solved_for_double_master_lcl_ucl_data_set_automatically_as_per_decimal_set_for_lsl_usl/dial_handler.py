...
class DialHandler(QObject):
    ...
    # Initialize dial rendering and spring animation state
    def __init__(self, main_window, needle_pos_x, needle_pos_y,
        ...
        try:
            ...
            print(f"Error initializing DialHandler: {e}")
            raise
        ...
        # Rotation can extend beyond the SVG item's previous bounding rect.
        # Repaint the viewport so the previous needle position is always erased.
        ...
        self.dial_pixmaps = {
            "green":  QPixmap(f"/home/torizon/app/src/images/dial-30-green-{self.size}.png"),
            "yellow": QPixmap(f"/home/torizon/app/src/images/dial-30-yellow-{self.size}.png"),
            "red":    QPixmap(f"/home/torizon/app/src/images/dial-30-red-{self.size}.png"),
        }
        ...
        self.spring_stiffness = 2.4#2.3
        self.spring_damping = 0.20#0.20
        self.velocity = 0
        self.target_angle = 0
    ...
    def set_dial_range(self, dial_range):
        try:
            #to set  range when range update 30 to 60 only ui change image as per range
            range_value = 30 if dial_range in (0.3, 0.03, 0.003) else 60

            # Don't reload if already correct
            if getattr(self, "current_range", None) == range_value:
                return

            self.current_range = range_value

            size = self.size   # save this in __init__

            self.dial_pixmaps = {
                "green":  QPixmap(f"/home/torizon/app/src/images/dial-{range_value}-green-{size}.png"),
                "yellow": QPixmap(f"/home/torizon/app/src/images/dial-{range_value}-yellow-{size}.png"),
                "red":    QPixmap(f"/home/torizon/app/src/images/dial-{range_value}-red-{size}.png"),
            }

            self.dial_item.setPixmap(self.dial_pixmaps[self.current_color])
        except Exception as e:
            print(f"Error in set_dial_range -> {e}")
    # ==================== UPDATE METHODS ====================
    # Update the dial image color based on the current status
    ...
    # Handle new needle angle requests and begin spring animation
    def _on_needle_angle_changed(self, angle):
        try:
            ...
            if not self.animation_timer.isActive():
                self.animation_timer.start(16)  # 60 FPS; avoids redundant GUI work
        except Exception as e:
            print(f"Error in needle angle change: {e}")
            self.is_animating = False
            self.animation_timer.stop()
    
    # Animate the needle using spring physics until the target angle is reached
    ...
    # Move the needle to a new angle via thread-safe signal emission
    def move_needle(self, angle):
        try:
            ...
            print(f"Error moving needle: {e}")
    
    # ==================== END MOVE NEEDLE ====================

    # Property (keeping QObject inheritance as requested)
    # Dial angle property used by Qt property system
    # Getter for the current needle angle
    @Property(float)
    def angle(self):
        try:
            return self.current_angle
        except Exception as e:
            print(f"Error reading dial angle: {e}")
            return 0.0
    
    # Setter to rotate the needle when angle changes
    @angle.setter
    def angle(self, value):
        try:
            ...
            print(f"Error setting dial angle: {e}")
