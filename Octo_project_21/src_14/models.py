from sqlalchemy import Column, Integer, String, Time, create_engine , Float
...
class UserManagement(Base):
    ...
    def insert_defaults(cls, session):
        ...
        defaults = [
            {"Username": "isense", "Password": "intersense", "AccessType": 0},
            {"Username": "admin", "Password": "admin", "AccessType": 1},
        ]
        for user in defaults:
            if not session.query(cls).filter_by(Username=user["Username"]).first():
...
class IOSettings(Base):
    ...
    def insert_defaults(cls, session):
        ...
        defaults = [
            ...
            {"Key": "DISPLAY_MODE", "Enable": None, "Value": "Digit"},
...
engine = create_engine("sqlite:////home/torizon/app/precigo.db")  # change to MSSQL if needed
