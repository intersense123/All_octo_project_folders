...
from cnc_Listmanager import CncListDisplay
from cnc_machine_Handler import CNCManager
from FanucLibDefs import FANUCDEFS
from statusBarmanager import PageStatusManager
...
class MainWindow(QMainWindow,Ui_MainWindow):
    def __init__(self):
        try:
            ...
            self.status_manager = PageStatusManager(
            ...
            self.cncListDisplay = CncListDisplay(self)
            self.cnc_manager = CNCManager(self)
            ...
            # Connect here
            self.controller.dimension_deleted.connect(
                self.dial_indicator.set_visibility
            )
            # ✅ Connect AFTER both objects are fully ready
            # self.data_handler.dialVisibilityChanged.connect(
            #     lambda: QTimer.singleShot(0, self.spc_manager.load_spc)
            # )
            # self.data_handler.dialVisibilityChanged.connect(
            #     lambda: QTimer.singleShot(0, self.dial_indicator.set_visibility)
            # )
            ...
            self.generate_Rs232()
            self.Gpio_handling()
            self.update_default_formula()
            # machine_one = FANUCDEFS('192.168.225.159',8193)
            ...
            # End initialization
            # self.machine_one = FANUCDEFS('192.168.225.159',8193)
            # print("1. FANUC object created")

            # ret = self.machine_one.focas.cnc_startupprocess(0, "focas.log")
            # print("2. startup ret =", ret)

            # if ret != 0:
            #     raise Exception(f"Failed to create required log file! ({ret})")
            # self.machine_one.connectFanuc()
            # print("3. Connected")

            # value = self.machine_one.readFanuc()
            # print("4. Machine ID:", value)

            # self.machine_one.disconnectMachine()
            # print("6. Done")
        except Exception as e:
            print(f"Error initializing MainWindow: {e}")
        
    def update_default_formula(self):
        try:
            if not self.label_formulaBar.text().strip():
                dim = self.comboBox_toselectProbe.currentText().strip()
                if dim:
                    self.label_formulaBar.setText(f"P{dim[1:]}")  
        except Exception as e:
            print(f"Error updating dafault formula: {e}")
    ...
    def Gpio_handling(self):
        ...
        try:
            ...
            self.gpio_handler.footswitch_pressed.connect(self.show_footswitch_indicator)
            self.gpio_handler.start()
            # print("GPIO Input Handler started")
        except Exception as e:
            self.gpio_handler = None
            print(f"Failed to initialize GPIO Handler: {e}")    
    def show_footswitch_indicator(self):
        try:
            # Show footswitch feedback safely
            self.label_savesignal.show()
            QTimer.singleShot(1000, self.label_savesignal.hide)
        except Exception as e:
            print(f"Error showing footswitch indicator: {e}")
    def open_factory_config(self):
        try:
            # Open factory configuration dialog safely
            self.factory_popup = FactoryConfig(parent=self,current_factory=self.spi_controller.current_factory)
            self.factory_popup.config_changed.connect(
                self.spi_controller.on_factory_changed
            )
            self.factory_popup.exec_()
        except Exception as e:
            print(f"Error opening factory configuration: {e}")
    ...
    def closeEvent(self, event):
        try:
            ...
            print(f"Error during close event cleanup: {e}")
        finally:
            event.accept()
