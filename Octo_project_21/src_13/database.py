...
class DatabaseAgent:
    ...
    def get_all_program_ids(self):
        """Return a list of all ProgramId values from ProgramSettings."""
        session: Session = self.SessionLocal()
        try:
            ids = session.query(ProgramSettings.ProgramId).all()
            # unpack from list of tuples -> [1, 2, 3, ...]
    ...
    def update_io_settings(
            ...
            with self.SessionLocal() as session:
                try:
                    ...
                    upsert("DISPLAY_MODE", DISPLAY_MODE)
    ...
    def upsert_user_management(self, username: str, password: str, access_type: str):
            """
            ...
            with self.SessionLocal() as session:
                try:
                    ...
                    if existing:
                        ...
                        existing.AccessType = access_type
                    else:
                        session.add(
                            UserManagement(
                                Username=username,
                    ...
                    session.rollback()
                    print(f"Error: User with username '{username}' already exists (IntegrityError).")
                    return False
                except SQLAlchemyError as e:
                    session.rollback()
                    print(f"Database error during upsert_user_management: {e}")
                    return False
                except Exception as e:
                    session.rollback()
    ...
    def upsert_program_settings(self, program_id: int, program_name: str, mode: str, uom: str, auto_save_duration: float):
            """
    ...
    #         try:
    #             # -------------------------------
    #             # 1️⃣ Always initialize structure
    #             # -------------------------------
    #             program_dict = {
    #                 "ProgramSpecificSettings": {
    #                     "ProgramId": program_id,
    ...
    #                 if not dim:
    #                     continue
    #                 #dim = f"D{probe.Dimension}".upper()   # 🔥 FIX HERE

    #                 if dim in program_dict:
    #                     program_dict[dim]["ProbeBasedSettings"] = {
    #                         "Formula": probe.Formula,
    #                         "ProgramName" : probe.ProgramName,
    #                         "MasterType": probe.MasterType,
