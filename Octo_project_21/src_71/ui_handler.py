...
class UIHandler(QObject):
    ...
    #     if self.ui.stacked.currentIndex() == 30:

    #         if self.ui.stackedWidget_2.currentIndex() == 4:
    ...
    def linkage_of_ui(self):
        try:
            ...
            self.ui.button_back.pressed.connect(lambda checked=False: (
                self.linkage_of_backButton(),
                self.handle_aocwidget_backButton()
            )
        )
            
            self.ui.button_forward.pressed.connect(lambda checked = False :(
                                                    self.linkage_of_forwardButton(),
                                                   self.handle_aocwidget_forwardButton())
            )
            
            self.ui.button_home.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(23))
            ...
            self.ui.button_partSettings.pressed.connect(self.open_part_settings)
            self.ui.button_programList.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(29))
            self.ui.button_staticSettingsForAll.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(26))
            self.ui.button_setFormulaSettings.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(4))
             
            self.ui.button_angleCalculationSetting.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(27))

            # IO Settings Page Clicked Slot
            # self.ui.label_ioSettingPage.mousePressEvent = lambda event: self.ui.stacked.setCurrentIndex(7)
            self.ui.button_databaseSettings.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(16))
            self.ui.button_rs232Settings.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(3))
            #self.ui.button_airSaving.clicked.connect(lambda : self.ui.stacked.setCurrentIndex(20))
            self.ui.button_ipSettings.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(5))
            self.ui.button_wifiSettings.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(6))
            self.ui.button_factoryCalibration.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(19))

            # AOC Page Clicked Slot
            self.ui.label_autoOffsetSettingPage.mousePressEvent = lambda event: self.ui.stacked.setCurrentIndex(14)
            self.ui.button_cncListAoc.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(12))
            self.ui.button_addCncSettings.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(13))
            self.ui.button_modifyCncSettings.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(13))
            self.ui.button_reportsAoc.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(18))
            ...
            self.ui.button_viewData.pressed.connect(lambda : self.ui.stacked.setCurrentIndex(22))
            self.ui.button_ListView.pressed.connect(lambda : self.ui.stackedWidget.setCurrentIndex(0))
            self.ui.button_ChartView.pressed.connect(lambda : self.ui.stackedWidget.setCurrentIndex(1))
            self.ui.button_HistogramChart.pressed.connect(lambda : self.ui.stackedWidget.setCurrentIndex(2))
    ...
    def powerButton(self):
        try:
            ...
            logout_action = self.ui.menu.addAction("Logout", self.logout)
            shutdown_action = self.ui.menu.addAction("Shutdown", self.shutdown)
            restart_action = self.ui.menu.addAction("Restart", self.restart)
            
            # Validate actions were added
            if not logout_action or not shutdown_action or not restart_action:
                print("Warning: Some menu actions failed to add")
