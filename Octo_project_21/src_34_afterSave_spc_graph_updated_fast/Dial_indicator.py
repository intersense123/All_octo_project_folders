...
class SPCBase:
    ...
    def __init__(self, title, lsl, lcl, nominal, ucl, usl):
        try:
            self.title = title                  # Graph title (D1, D2, ...)
            self.limits = [lsl, lcl, nominal, ucl, usl]                # SPC limits

            self.data_raw = []                  # Raw input values
            self.data_scaled = []               # Scaled values for plotting

            self.y_positions = np.arange(5)     # 7 SPC zones
            
            self.spc_count = 50

            # Colors and styles for limit lines
            self.line_colors = ["red","green","green","green","red"]
            self.line_styles = ["--",":","-.",":","--"]
    ...
    def configure_x_axis(self):
        try:
            ...
            self.ax.set_xticks(np.arange(1, self.spc_count + 1, 3))
            self.ax.set_xticklabels(
                np.arange(1, self.spc_count + 1, 3),
                color="white"
            )
        except Exception as e:
            print(f"Error in configure_x_axis -> {e}")
    ...
    def scale_value(self, v):
        try:
            ...
            return 0 if v < self.limits[0] else 4
        except Exception as e:
    ...
    def draw_background(self):
        try:
            ...
            self.ax.axhspan(1, 2, color="green", alpha=0.9)
            self.ax.axhspan(2, 3, color="green", alpha=0.9)
            self.ax.axhspan(3, 4, color="red", alpha=0.9)
            # self.ax.axhspan(4, 5, color="red", alpha=0.9)
    ...
    def draw_limit_lines(self):
        try:
            xmax = self.spc_count#len(self.data_raw) if self.data_raw else 1
            for i in range(5):
                self.ax.hlines(
                    self.y_positions[i],
                    xmin=0,
    ...
    def set_limits(self, lsl, lcl, nominal, ucl, usl):
        try:
            self.limits = [lsl, lcl, nominal, ucl, usl]
    ...
    def init_blank_ui(self):
        try:
            ...
            self.line, = self.ax.plot([], [], color="blue", marker="o", markersize=6, linewidth=2)
    ...
    def generate_graph(self, raw_data):
        ...
        self.line.set_data(x, self.data_scaled)
    ...
    def update_graph(self, value):
        ...
        # scale
        self.data_scaled = [self.scale_value(v) for v in self.data_raw]
...
class SPCManager:
    def __init__(self, main_window):
        try:
            ...
            self.spc_all = {
            1: SPCWidget(SPCBase("D1", lsl=20.010,lcl=20.015,nominal=20.020,ucl=20.025,usl=20.030)),
            2: SPCWidget(SPCBase("D2", lsl=20.010,lcl=20.015,nominal=20.020,ucl=20.025,usl=20.030)),
            3: SPCWidget(SPCBase("D3", lsl=20.010,lcl=20.015,nominal=20.020,ucl=20.025,usl=20.030)),
            4: SPCWidget(SPCBase("D4", lsl=20.010,lcl=20.015,nominal=20.020,ucl=20.025,usl=20.030)),
            }
            ...
            # channels = self.ui.controller.get_channels_for_program(
            # int(self.ui.valueObj.activeVariables_dict["ActiveProgramId"])
            # )
    ...
    def load_program_OnSpc(self, program_id: int):
        ...
        try:
            ...
            session.close()
    def load_previous_spc_values(self, channels):
        try:
    ...
    def apply_layout(self):
        try:
            # Check if layouts exist, if not create them
            ...
            # ---------------- 2 SPC ----------------
                ...
                if self.spc_1 is not None:
                    self.spc_1.addWidget(spcs[0])
                    spcs[0].spc.canvas.setMinimumHeight(120)

                self.ui.widget_spc1.show()
                self.ui.widget_spc1.setGeometry(0, -10, 771, 230)

                # -------- BOTTOM SPC --------
                if self.spc_3 is not None:
                    self.spc_3.addWidget(spcs[1])
                    spcs[1].spc.canvas.setMinimumHeight(120)

                self.ui.widget_spc3.show()
                self.ui.widget_spc3.setGeometry(10, 180, 771, 230)
