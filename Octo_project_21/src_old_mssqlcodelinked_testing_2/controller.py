...
from models import *
import models
from save_handler import CSVManager
...
class AppController(QObject):
    ...
    def get_channels_for_program(self, program_id):
        """
        Returns a set of channels like {'D1', 'D2'}
        for the given ProgramId
        """
        ...
        return {
            key for key in self.ui.valueObj.ProgramSettings_dict.keys()
            if key.startswith("D")
        }
    ...
    def linkage_of_saveButton(self):
        try:
            ...
            if current_index == 6:
                ...
                self.save_probe_values_to_csv() #To save on network or windows
    ...
    # -------- SAVE PROBE VALUES TO CSV --------
    def save_probe_values_to_csv(self):
        """
        Save probe values from label_value1-4 and status1-4 to CSV
        Called when clicking Save button at index 30
        """
        try:
            # Initialize CSV Manager
            csv_manager = CSVManager()
            
            # Get probe values from labels
            probe_values = {
                "D1": self.ui.label_value1.text(),
                "D2": self.ui.label_value2.text(),
                "D3": self.ui.label_value3.text(),
                "D4": self.ui.label_value4.text()
            }
            
            # Get status from labels
            probe_status = {
                "D1": self.ui.label_status1.text(),
                "D2": self.ui.label_status2.text(),
                "D3": self.ui.label_status3.text(),
                "D4": self.ui.label_status4.text()
            }
            ...
            # Get job count (from active variables or use default)
            job_count = int(
                self.ui.valueObj.activeVariables_dict.get("JobCount", 1)
            )
            
            # Get channel/mode settings
            program_specific_settings = self.ui.valueObj.ProgramSettings_dict.get('ProgramSpecificSettings', {})
            mode = program_specific_settings.get("Mode", "Individual")
            
            # Get probe data from database for each dimension
            with SessionLocal() as session:
                for probe, value in probe_values.items():
                    if not value or value == "0":
                        continue
                    
                    # Get probe settings from database
                    db_record = session.query(ProbeBasedSettings).filter(
                        ProbeBasedSettings.ProgramId == active_program_id,
                        ProbeBasedSettings.Dimension == probe
                    ).first()
                    
                    if db_record:
                        status = probe_status.get(probe, "OK")
                        
                        # Save to CSV (and MSSQL if available)
                        csv_manager.save_live_data(
                            db_record=db_record,
                            value=float(value),
                            status=status,
                            jobcount=job_count,
                            channel=int(probe[1:]),  # Extract channel number from D1, D2, etc.
                            mode=mode.lower() if mode else "individual",
                            aoc_record=None  # Pass None if AOC not selected
                        )
                        
                        print(f"Saved {probe}: value={value}, status={status}")
            
            print("Probe values saved to CSV successfully!")
            
        except Exception as e:
            print(f"Error saving probe values to CSV: {e}")
