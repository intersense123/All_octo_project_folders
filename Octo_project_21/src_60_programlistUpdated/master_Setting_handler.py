    ...
    def ok_notok_part_check(self):
        try:
            ...
            for probe, pdata in self.probe_data.items():
                ...
                calculated_values[probe] = Final_value
            ...
            #             if label:
            #                 label.setText(self.format_by_leastcount(final_value, leastcount))
            ...
            for probe, pdata in self.probe_data.items():
                ...
                # -------------------------------
                # 🔹 ALWAYS SHOW VALUE (instant)
                # -------------------------------
                if label:
                    label.setText(self.format_by_leastcount(final_value, leastcount))

                # -------------------------------
                # 🔹 STEP 1: RAW STATUS (instant)
                # -------------------------------
                if pdata["method"] == "OD":
                    if pdata["USL"] < final_value:
                        raw_status = "REWORK"
                ...
                if self.last_raw_status.get(probe) != raw_status:
                    ...
                    # show waiting indicator (optional)
                    if status_label:
                        status_label.setText("")
                        status_label.setStyleSheet(
                ...
                if elapsed < self.auto_save_duration:
                    continue

                # -------------------------------
                # 🔹 STEP 4: FINAL STABLE STATUS
                # -------------------------------
                status = raw_status
                self.last_status[probe] = status
            ...
            if self.relay_state == '1' and self.last_status:
                ...
                # 🔥 detect change
                if self.last_checked_relay_color != relay_color:
                    self.last_checked_relay_color = relay_color
                    self.last_trigger_time = current_time
                    return
                ...
                if elapsed >= self.auto_save_duration:
                    ...
                    if self.last_relay_color != relay_color:
                        self.ui.relayController.trigger(relay_color, self.relay_value)
