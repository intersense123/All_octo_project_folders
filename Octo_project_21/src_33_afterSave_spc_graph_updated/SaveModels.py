...
class SaveDatabaseManager:
    ...
    # def get_probe_values_for_spc(self, probe_ids, limit=50):
    ...
    #     try:
    #         placeholders = ",".join([":p" + str(i) for i in range(len(probe_ids))])

    #         query = text(f"""
    #         SELECT ProbeBasedId, Value
    #         FROM ProbeValues
    #         WHERE ProbeBasedId IN ({placeholders})
    #         ORDER BY GlobalCounter DESC
    #         LIMIT :limit
    #         """)

    #         params = {f"p{i}": probe_ids[i] for i in range(len(probe_ids))}
    #         params["limit"] = limit
