...
class DataHandler(QObject):
    ...
    def load_ProbeBasedSettings_to_ui(self,dimension):
        try:
            ...
            # formula = str(probe_settings.get("Formula", "")).strip()

            # # If no formula is saved, default to corresponding probe
            # if not formula:
            #     if dimension.upper().startswith("D"):
            #         dim_no = dimension[1:]   # D1 -> 1
            #         formula = f"P{dim_no}"

            # self.ui.label_formulaBar.setText(formula)
    ...
    def load_IOSettings_to_ui(self):
        try:
            ...
            # # SAVE MASTER CALIBRATION
            # if io_dict.get("SAVE_MASTER_CALIBRATION", {}).get("Enable") == "ON":

            #     self.ui.radioButton_saveMasterDbON.setChecked(True)
            #     self.ui.radioButton_saveMasterDbOFF.setChecked(False)

            # else:

            #     self.ui.radioButton_saveMasterDbOFF.setChecked(True)
            #     self.ui.radioButton_saveMasterDbON.setChecked(False)
    ...
    def save_toDatabase_IOSettings(self):
        try:
            ...
            savemastercalibration_enable = self.ui.valueObj.IOSettings_dict.get('SAVE_MASTER_CALIBRATION', {}).get('Enable', 'OFF')
    ...
    def save_toDatabase_userManagement(self):
        ...
        try:
            ...
            # self.ui.status_manager.show(
            #     page_index=8,
            #     message="User Setting Loaded Successfully.",
            #     timeout=0
            # )            
