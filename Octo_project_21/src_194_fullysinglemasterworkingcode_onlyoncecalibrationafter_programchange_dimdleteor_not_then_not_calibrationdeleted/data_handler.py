...
class DataHandler(QObject):
    ...
    def __init__(self, main_window,uiHandler):
        try:
            ...
            self.master_name_labels = {
                "D1": {
                    "high": self.ui.label_masterD1_2,
                    "low":  self.ui.label_masterD1_1,
                },
                "D2": {
                    "high": self.ui.label_masterD2_2,
                    "low":  self.ui.label_masterD2_1,
                },
                "D3": {
                    "high": self.ui.label_masterD3_2,
                    "low":  self.ui.label_masterD3_1,
                },
                "D4": {
                    "high": self.ui.label_masterD4_2,
                    "low":  self.ui.label_masterD4_1,
                },
            }
    ...
    def calculate_control_limits(self):
        ...
        try:
            ...
            # Get decimal resolution from LSL and USL
            def get_decimal_places(value_text):
                if "." in value_text:
                    return len(value_text.split(".")[1])
                return 0

            lsl_decimals = get_decimal_places(lsl_text)
            usl_decimals = get_decimal_places(usl_text)

            # Use the highest resolution
            decimals = max(lsl_decimals, usl_decimals)

            # Calculate control limits
            lcl = nominal - ((nominal - lsl) * 0.70)
            ucl = nominal + ((usl - nominal) * 0.70)

            # Format dynamically according to input resolution
            if not self.ui.lineEdit_lcl.text().strip():
                self.ui.lineEdit_lcl.setText(f"{lcl:.{decimals}f}")

            if not self.ui.lineEdit_ucl.text().strip():
                self.ui.lineEdit_ucl.setText(f"{ucl:.{decimals}f}")
    ...
    def save_toDatabase_userManagement(self):
        try:
            ...
            if user_id is not None and user_id == user_table.protected_admin_user_id:
                session = SessionLocal()
                current_user = session.query(UserManagement).filter(
                    UserManagement.id == user_id
                ).first()
                session.close()
                if current_user:
                    setUsername = current_user.Username
                    setAccessType = current_user.AccessType
            ...
            user_table.protected_admin_user_id = None
    ...
    def clear_master_values_ui(self):
        try:
            ...
            # Also restore all master title labels
            for dim, labels in self.master_name_labels.items():
                labels["low"].setText(f"MasterLower {dim} :")
                labels["high"].setText(f"MasterHigher {dim} :")
    ...
    def load_Higher_lower_value_to_ui(self, program_id):
        try:
            # ==========================================
            # CLEAR OLD VALUES / HIDE ALL ROWS
            # ==========================================
            ...
            with self.ui.databaseObj.SessionLocal() as session:
                ...
                for probe in probes:
                    ...
                    # VALUE LABELS
                    # ==========================================
                    value_labels = self.master_value_labels[dim]

                    lbl_low_value = value_labels["low"]
                    lbl_high_value = value_labels["high"]

                    # ==========================================
                    # TITLE LABELS
                    # ==========================================
                    name_labels = self.master_name_labels[dim]

                    lbl_low_name = name_labels["low"]
                    lbl_high_name = name_labels["high"]

                    # ==================================================
                    # SINGLE MASTER
                    # ==================================================
                    if probe.MasterType == "Single Master":

                        # print(
                        #     f"[SINGLE MASTER UI] "
                        #     f"{dim} -> Master = {probe.Master}"
                        # )

                        # ------------------------------------------
                        # Change LOWER title to MASTER
                        # ------------------------------------------
                        lbl_low_name.setText(
                            f"Master {dim} :"
                        )

                        # ------------------------------------------
                        # Show actual Master DB value
                        # in the SAME position as lower value
                        # ------------------------------------------
                        lbl_low_value.setText(
                            "" if probe.Master is None
                            else str(probe.Master)
                        )

                        # ------------------------------------------
                        # Hide HIGH title + value
                        # ------------------------------------------
                        lbl_high_name.hide()
                        lbl_high_value.hide()

                        # Make sure LOW/Master is visible
                        lbl_low_name.show()
                        lbl_low_value.show()
                    ...
                    # ==================================================
                        ...
                        # ------------------------------------------
                        lbl_low_value.setText(
                            "" if probe.MasterLower is None
                            else str(probe.MasterLower)
                        )

                        # ------------------------------------------
                        # HIGH value
                        # ------------------------------------------
                        lbl_high_value.setText(
                            "" if probe.MasterHigher is None
                            else str(probe.MasterHigher)
                        )
            ...
            print(
                f"Error in load_Higher_lower_value_to_ui: {e}"
            )
    # def load_Higher_lower_value_to_ui(self, program_id):
    #     try:
    ...
    #                     print(
    #                         f"[SINGLE MASTER UI] {dim} "
    #                         f"Master = {probe.Master}"
    #                     )
    ...
    #     except Exception as e:
    #         print(f"Error in load_Higher_lower_value_to_ui: {e}")
    ...
    def delete_dimension(self, program_id, dimension):
        try:
            with SessionLocal() as session:
                try:
                    probe_settings = session.query(ProbeBasedSettings).filter_by(
                        ProgramId=program_id,
                        Dimension=dimension
                    ).first()

                    is_single_master = (
                        probe_settings is not None
                        and probe_settings.MasterType == "Single Master"
                    )
                    ...
                    # 🔥 CHANGED: don't wipe factory LOW/HIGH calibration for a
                    # Single Master probe just because its Dimension label is
                    # being deleted — the physical probe's calibration should
                    # survive a dimension reassignment.
                    if not is_single_master:
    ...
    #     """
    #     Deletes entire program and all its related data safely. but singlemaster 
    #       calibration data remain constant for that probe
    #     """
    #     try:
    #         with SessionLocal() as session:
    #             try:
    ...
    #                     row.Dimension
    #                     for row in session.query(ProbeBasedSettings).filter(
    #                         ProbeBasedSettings.ProgramId == program_id,
    #                         ProbeBasedSettings.MasterType == "Single Master"
    #                     ).all()
    #                 ]
    ...
    #                 master_query = session.query(SetMasterSettings).filter(
    #                     SetMasterSettings.ProgramId == program_id
    #                 )
    #                 if single_master_dimensions:
    #                     master_query.filter(
    #                         ~SetMasterSettings.Dimension.in_(single_master_dimensions)
    #                     ).delete(synchronize_session=False)
    #                 else:
    #                     master_query.delete(synchronize_session=False)
    #                 session.commit()
    ...
    def delete_program(self, program_id):
        ...
        try:
            with SessionLocal() as session:
                try:
                    # 🔥 Find which dimensions in this program are Single Master
                    # BEFORE deleting ProbeBasedSettings — we need this to decide
                    # which SetMasterSettings rows must be preserved.
                    single_master_dims = [
                        row.Dimension for row in session.query(ProbeBasedSettings).filter(
                            ProbeBasedSettings.ProgramId == program_id,
                            ProbeBasedSettings.MasterType == "Single Master"
                        ).all()
                    ]
                    ...
                    # 🔥 CHANGED: don't delete SetMasterSettings rows that hold
                    # factory LOW/HIGH calibration for a Single Master probe —
                    # that calibration belongs to the physical probe, not to this
                    # program, and must survive the program being deleted.
                    query = session.query(SetMasterSettings).filter(
                        SetMasterSettings.ProgramId == program_id
                    )
                    if single_master_dims:
                        query = query.filter(
                            ~SetMasterSettings.Dimension.in_(single_master_dims)
                        )
            ...
            if hasattr(self, "selected_program_id"):
                ...
                if program_id is not None:
                    confirm_msg = CustomMessageBox(
                        ...
                        "warning" if str(program_id) == "1" else "info",
                        parent=self.ui
                    )
                    confirm_msg.exec_()
