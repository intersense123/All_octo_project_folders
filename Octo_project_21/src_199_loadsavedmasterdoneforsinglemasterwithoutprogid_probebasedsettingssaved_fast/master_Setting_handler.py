...
class SPIController(QObject):
    ...
    def dimension_cache_status_clear(self, dimension):
        try:
            ...
            if status_label:
                status_label.clear()
                status_label.setStyleSheet(self.ui.dial_indicator.style_status)

            value_label = self.probe_labels.get(dimension)
            if value_label:
                value_label.setStyleSheet(self.ui.dial_indicator.style_value)
            # -------------------------------------------------
        ...
        try:
            ...
            for probe, pdata in no_master_limits.items():
                ...
                if raw_value is None or raw_value == 0:
                    # A direct copy such as D4 = D1 may temporarily have no
                    # calculated source after another channel's master setup.
                    # The raw UI can still show its default 0.000, but that is
                    # not a measurement and must not retain an old status.
                    label = self.probe_labels.get(probe)
                    if label:
                        label.setStyleSheet(
                            self.ui.dial_indicator.style_value +
