...
class SaveDatabaseManager:
    ...
    def __init__(self, db_url=None):
        ...
        class AOCValues(self.Base):
            ...
            Dimension = Column(String(5), nullable=False)
    ...
    def get_job_counts(self):
        ...
        try:
            rows = session.execute("""
                SELECT pv.ProbeBasedId, MAX(pv.JobCount)
                FROM ProbeValues AS pv
                LEFT JOIN SPCResetTracker AS reset
                    ON reset.ProbeBasedId = pv.ProbeBasedId
                WHERE reset.LastResetCounter IS NULL
                   OR pv.GlobalCounter > reset.LastResetCounter
                GROUP BY pv.ProbeBasedId
            """).fetchall()
