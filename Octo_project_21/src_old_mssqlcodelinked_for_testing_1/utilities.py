...
class Utilities:
    def __init__(self,main_window) : # label_time, label_date, lineEdit_formulaBar):
        self.ui = main_window
        #self.ui.label_time = label_time
        # self.ui.label_date = label_date
        # self.ui.lineEdit_formulaBar = lineEdit_formulaBar
        self.timer = None
    ...
    def validate_formulabar(self):
        ...
        # formula = re.sub(r'P\d+', '2.5', self.ui.lineEdit_formulaBar.text())
        # formula_1 = re.sub(r'D\d+', '2.5', self.ui.lineEdit_formulaBar.text())
    ...
    def set_system_time_imx7(self):
        ...
        try:
            ...
            time_str = f"{year}-{month:02d}-{day:02d} {hour:02d}:{minute:02d}:00"
            cmd = f'date -s "{time_str}"'
            print("User ID:", os.geteuid())
            result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
            # subprocess.run(["hwclock", "-w", "-f", "/dev/rtc1"], check=True)
            # hcmd = f'hwclock -w -f /dev/rtc1'
             # save to hardware RTC
            # subprocess.run(hcmd, shell=True, capture_output=True, text=True)

            if result.returncode == 0:
                self.update_display()
                print("✅ Time updated:", time_str)
            else:
