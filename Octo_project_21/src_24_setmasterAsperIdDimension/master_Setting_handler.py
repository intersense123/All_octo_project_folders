...
class SetMasterController(QObject):
    ...
    #     """
    #     Get DB values for a specific probe from ProbeBasedSettings table.
    #     Returns dict with DB values or empty dict if not found.
    #     """
    #     try:
    ...
    def ok_notok_part_check(self):
        ...
        try:
            ...
            for probe, pdata in self.probe_data.items():
                ...
                if(pdata["method"] == 'OD'):
                    if(pdata["USL"] < final_value_for_check):
                        status = 'REWORK'
                    elif(pdata["LSL"] > final_value_for_check):
                        status = 'NOT OK'
                    else:
                        status = 'OK'
                ...
                if probe == "D1":
                    ...
                    if '3' in str(range):
                        value_of_tick_micron = range / 30
                        angle_of_tick = 90 / 30
                    elif '6' in str(range):
                        value_of_tick_micron = range / 60
                        angle_of_tick = 90 / 60

                    value_differed_from_nominal = final_value_for_check - nominal

                    angle = (value_differed_from_nominal / value_of_tick_micron) * angle_of_tick
                    angle = max(-90, min(90, angle))

                        # normalized = (final_value_for_check - lsl) / (usl - lsl)
                        # angle = -90 + (normalized * 180)  # -90 to +90
                        # angle = max(-90, min(90, angle))  # Clamp
                    # else:
                    #     angle = 0
                    
                    # thread = threading.Thread(target=self.ui.dial_1_anim.move_needle, args=(angle,))
                    # thread.daemon = True
                    # thread.start()
                ...
                # # Check if fixture is empty (part removed) by comparing with baseline
                if autoSense_value is not None:
