...
class SPIController(QObject):
    ...
    def save_probe_values(self):
        # To save live values and status on ProbeValues table
        ...
        try:
            ...
            global_counter, job_count = self.ui.savedbObj.get_next_counters()
            # job_count = 1
            ...
            if mode == "Combine":
                ...
                for dim, status_label in self.status_labels.items():

                    if status_label.text().strip() == "":

                        msg = CustomMessageBox(
                            "Status missing. Cannot save in Combine mode.",
                            "error",
                            parent=self.ui
                        )
                        msg.exec_()
            ...
            # SAVE VALUES
            # --------------------------------
            for dim, value_label in self.probe_labels.items():

                value_text = value_label.text().strip()
                status_text = self.status_labels[dim].text().strip()
                ...
                # 🔴 DEBUG PRINT
                # print(
                #     f"SAVING -> Dimension:{dim}, "
                #     f"ProbeId:{probe_id}, "
                #     f"Value:{value}, "
                #     f"Status:{status_text}, "
                #     f"GlobalCounter:{global_counter}, "
                #     f"JobCount:{job_count}, "
                #     f"Mode:{mode_char}"
                # )
