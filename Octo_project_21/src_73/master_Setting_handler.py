...
class SPIController(QObject):
    ...
    def __init__(self, main_window, data_handler,app_controller):
        try:
            ...
            self.last_values = {}   # store last values per probe
    ...
    def ok_notok_part_check(self):
        try:
            ...
            VALUE_CHANGE_TOL = 0.000001

            channel_count = self.ui.dial_indicator.channel_count

            font_size = (
                "50pt" if channel_count == 1
                else "45pt" if channel_count == 2
                else "35pt"
            )
            ...
            if not hasattr(self, "autosense_state"):
                self.autosense_state = {}
            ...
            if formula_list:
                ...
                formula_results = self.set_per_formula(
                    a,
                    b,
                    c,
                    d,
            ...
            for probe, pdata in self.probe_data.items():
                ...
                current_autosense = False

                if autoSense_value is not None and spi_new is not None:

                    if abs(
                        spi_new - autoSense_value
                    ) < AUTOSENSE_TOLERANCE_RANGE:

                        current_autosense = True

                prev_autosense = self.autosense_state.get(
                    probe,
                    False
                )

                self.autosense_state[probe] = current_autosense

                # -------------------------------------------------
                # AUTOSENSE DISPLAY
                # -------------------------------------------------
                if current_autosense:

                    # first entry into autosense
                    if not prev_autosense:
                    ...
                    # IMPORTANT
                    # autosense ignored in compare
                    self.last_values[probe] = None
                ...
                self.last_values[probe] = final_value
            ...
            for probe, status in self.last_status.items():
                ...
                # ignore autosense probes
                if self.last_values.get(probe) is None:
                    continue

                prev_status = self.previous_status.get(probe)
            ...
            for probe, value in self.last_values.items():

                # ignore autosense
                if value is None:
                    continue

                prev_value = self.previous_values.get(probe)

                # first cycle ignore
                if prev_value is None:
                    continue

                if abs(prev_value - value) > VALUE_CHANGE_TOL:
            ...
            if self.relay_state == '1' and self.last_status:
                ...
                relay_color = self.get_relay_color(
                    self.last_status
                )
                ...
                if not hasattr(
                    self,
                    "relay_triggered_for_cycle"
                ):

                    self.relay_triggered_for_cycle = False

                # restart cycle
                if status_changed or value_changed:
            ...
            self.previous_values = self.last_values.copy()
