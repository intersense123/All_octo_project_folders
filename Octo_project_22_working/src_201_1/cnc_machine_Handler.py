...
from aoc_csv_exporter import AOCExport
import subprocess
...
from Calender_dialog import AOCDateCalendar
# try:
...
class CNCManager(QObject):
    ...
    def __init__(self, main_window):
        try:
            ...
            self.ui = main_window
            self.aoc_export = AOCExport()
            self.setup_aoc_report_export()
            ...
            self.show_message_signal.connect(self.show_message_on_main_thread)

            self.connection_thread = None

            
            # GPIO pins for live machine selection (same pins used for READING_FREEZE scenario elsewhere)
    ...
    def show_message_on_main_thread(self, text, msg_type):
        msg = CustomMessageBox(text, msg_type, parent=self.ui)
        msg.exec_()

    def setup_aoc_report_export(self):
        try:
            """Connect AOC report controls on the main window."""
            self.ui.label_fromDate.setCursor(Qt.PointingHandCursor)
            self.ui.label_toDate.setCursor(Qt.PointingHandCursor)
            # self.ui.label_fromDate.mousePressEvent = lambda event: self.select_aoc_report_date("from")
            # self.ui.label_toDate.mousePressEvent = lambda event: self.select_aoc_report_date("to")
            self.ui.button_fromDate.clicked.connect(lambda: self.select_aoc_report_date("from"))
            self.ui.button_toDate.clicked.connect(lambda: self.select_aoc_report_date("to"))
            self.ui.button_export.clicked.connect(self.export_aoc_report)
        except Exception as e:
            print(f"Error in setup_aoc_report_export -> {e}")
    
    def select_aoc_report_date(self, date_type):
        try:
            dialog = AOCDateCalendar(
                date_type,
                self.ui
            )
            ...
            print(
                "Error opening AOC calendar:",
                e
            )
    def export_aoc_report(self):
        """Export the selected program and date range to USB as an XLSX workbook."""
        try:
            from_date = datetime.strptime(self.ui.label_fromDate.text().strip(), "%d/%m/%Y").date()
            to_date = datetime.strptime(self.ui.label_toDate.text().strip(), "%d/%m/%Y").date()
            program_id = self.ui.comboBox_progid_for_aocreports.currentText().strip()
            if not program_id:
                raise ValueError("Select a Program ID before exporting.")
            output_path, row_count = self.aoc_export.export_filtered_report(
                program_id, 
                from_date, 
                to_date
            )
            if output_path is None:
                self.show_message_signal.emit("No AOC CSV data matched the selected program and dates.", "warning")
                return
            ...
            # self.show_message_signal.emit(
            #     "Exported {} to {}".format(row_count, output_path), "success"
            # )
        except PermissionError:
            message = (
                "USB drive detected, but it is not writable. "
                "Check the USB drive mount permissions and try again."
            )
            print("Error exporting AOC report -> {}".format(message))
            self.show_message_signal.emit(message, "error")
        except Exception as error:
            print("Error exporting AOC report -> {}".format(error))
            self.show_message_signal.emit("AOC export failed: {}".format(error), "error")
    def ping_button_clicked(self):
        try:
            ...
            print(f"Error in ping_button_clicked -> ",e)    
    ...
    def get_live_selection(self):
        ...
        (27, 17, 24, 25) that are also used for the READING_FREEZE scenario elsewhere in the app.
        """
        try:
            # if GPIO is None:
    ...
    def save_aoc_values(
        ...
        """
        Persist one reading/correction cycle to Dial_indicator.

        If correction is a special value:
            999.999
            999.998
            999.997

        then CSV will save correction as 0.000.
        """
        ...
        try:
            ...
            # =====================================================
            # DATABASE RECORD
            # Original correction value is preserved in DB
            # =====================================================
            record = (
                aoc_id,
            ...
            # =====================================================
            # CSV CORRECTION VALUE
            # Convert special correction values to 0.000
            # =====================================================
            csv_correction = correction

            try:
                correction_value = float(correction)

                if correction_value in (
                    999.999,
                    999.998,
                    999.997
                ):
                    csv_correction = 0.000

            except (ValueError, TypeError):
                # If correction is not numeric, keep original value
                csv_correction = correction
            ...
            try:
                ...
                self.aoc_export.export_to_csv(
                    Sr_No=self.aoc_global_counter,
                    ...
                    Nominal=nominal,
                    Readings=offs,
                    Correction=csv_correction,
                    Tool_value=tool_value,
                    
                )

            except Exception as csv_error:
                # CSV logging must not interrupt database correction flow.
                print(
                    f"Error saving AOC correction CSV -> {csv_error}"
                )
