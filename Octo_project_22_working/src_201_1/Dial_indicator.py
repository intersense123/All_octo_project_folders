    ...
    def run(self):
        ...
        try:            
            ...
            for row in probe_rows:
                ...
                aoc_enable = row_aoc.aocEnable if row_aoc else "OFF"

                if self._aoc_state == "ON" and row_aoc and aoc_enable == "ON": 
...
class SPCBase:
    def __init__(self, title, lol, lsl, lcl, nominal, ucl, usl, uol, least_count):
        try:
            ...
            self.build_static_axes()
            self.build_limit_artists()
            # self.canvas.draw()          # ← FIX 4: non-blocking
            self.background = None
            self.cache_background()
        except Exception as e:
            print(f"Error in SPCBAse init -> {e}")
    # ── private ──────────────────────────────────────────────────────────────
    def set_title(self, title):
        ...
        # self.cache_background()   # redraw
    def cache_background(self):
        """
        Cache only the static graph.
        Then immediately redraw the current blue SPC line.
        """
        try:
            # Save current state
            line_visible = self.line.get_visible()

            # Hide dynamic line before making background cache
            self.line.set_visible(False)

            # Full draw of only static artists
            self.canvas.draw()
            ...
            self.line.set_visible(line_visible)

            # IMPORTANT:
            # Draw the existing line immediately.
            # Without this, line appears only after next update_graph().
            if line_visible and len(self.data_raw) > 0:
                self.canvas.restore_region(self.background)
                self.ax.draw_artist(self.line)
                self.canvas.blit(self.ax.bbox)

        except Exception as e:
            print(f"Error in cache_background -> {e}")
            
    def build_static_axes(self):
        try:
            ax = self.ax
            # Keep the first and last marker fully inside the axes.
            ax.set_xlim(0.5, self.spc_count + 0.5)
            ax.set_xticks(self.dynamic_xticks())
            ...
            self.line, = ax.plot(
                ...
                marker          = "o",
                markersize      = 6,           #6 was 3 — too small for 100-130px chart height
                linewidth       = 1.5,
            ...
            print(f"Error in build_static_axes --> {e}")
 
    def build_limit_artists(self):
        try:
            ax = self.ax
    
            # horizontal lines — zorder 5 so they sit above spans but below data
            self.hlines["usl"]     = ax.axhline(0, color='red',   linestyle='--', linewidth=1.5, visible=False, zorder=5)
            self.hlines["lsl"]     = ax.axhline(0, color='red',   linestyle='--', linewidth=1.5, visible=False, zorder=5)
            self.hlines["nominal"] = ax.axhline(0, color='green', linestyle='--', linewidth=1.5, visible=False, zorder=5)
            self.hlines["ucl"]     = ax.axhline(0, color='green', linestyle=':',  linewidth=1.5, visible=False, zorder=5)
            self.hlines["lcl"]     = ax.axhline(0, color='green', linestyle=':',  linewidth=1.5, visible=False, zorder=5)
            self.hlines["uol"]     = ax.axhline(0, color='red',   linestyle='-.', linewidth=1.5, visible=False, zorder=5)
            self.hlines["lol"]     = ax.axhline(0, color='red',   linestyle='-.', linewidth=1.5, visible=False, zorder=5)
    
            # spans — zorder 1 (bottom layer)
            self.hspans["lol_lsl"] = ax.axhspan(0, 1, color='red',    alpha=0.9, visible=False, zorder=1)
            self.hspans["usl_uol"] = ax.axhspan(0, 1, color='red',    alpha=0.9, visible=False, zorder=1)
            self.hspans["lsl_lcl"] = ax.axhspan(0, 1, color='yellow', alpha=0.9, visible=False, zorder=1)
            self.hspans["lcl_nom"] = ax.axhspan(0, 1, color='green',  alpha=0.9, visible=False, zorder=1)
            self.hspans["nom_ucl"] = ax.axhspan(0, 1, color='green',  alpha=0.9, visible=False, zorder=1)
            self.hspans["ucl_usl"] = ax.axhspan(0, 1, color='yellow', alpha=0.9, visible=False, zorder=1)
    
            self.update_limit_artists()
        except Exception as e:
            print (f"Error in build_limit_artists ->{e}")
            
    def update_limit_artists(self):
        try:
            lol, lsl, lcl, nominal, ucl, usl, uol = self.limits

            def hline(key, y):
                self.hlines[key].set_ydata([y, y])
                self.hlines[key].set_visible(True)

            def span(key, y0, y1, visible=True):
                self.hspans[key].set_xy([[0, y0], [1, y0], [1, y1], [0, y1], [0, y0]])
                self.hspans[key].set_visible(visible)

            hline("usl", usl);   hline("lsl", lsl)
            hline("nominal", nominal)
            hline("ucl", ucl);   hline("lcl", lcl)

            has_outer = (uol != 0 and lol != 0)
            self.hlines["uol"].set_ydata([uol, uol]); self.hlines["uol"].set_visible(has_outer)
            self.hlines["lol"].set_ydata([lol, lol]); self.hlines["lol"].set_visible(has_outer)

            span("lsl_lcl", lsl,  lcl);     span("lcl_nom", lcl,  nominal)
            span("nom_ucl", nominal, ucl);  span("ucl_usl", ucl,  usl)
            span("lol_lsl", lol, lsl, visible=has_outer)
            span("usl_uol", usl, uol, visible=has_outer)

            # Tick positions = actual saved limit values
            yticks = [lsl, lcl, nominal, ucl, usl]

            # Labels displayed on graph
            ylabels = ["LSL", "LCL", "MEAN", "UCL", "USL"]

            if has_outer:
                yticks.insert(0, lol)
                yticks.append(uol)

                ylabels.insert(0, "LOL")
                ylabels.append("UOL")
            ...
            # The data can be clamped exactly to LOL/UOL.  Leave enough room
            # for the marker diameter so those boundary points are not clipped.
            pad = max(abs(eff_uol - eff_lol) * 0.04, self.least_count * 2)
            self.ax.set_ylim(eff_lol - pad, eff_uol + pad)
        except Exception as e:
            print(f"Error in update_limit_artists -> {e}")
    
    def dynamic_xticks(self, current_count=None):
        try:
            # Calculate X-axis tick positions dynamically
            ...
            # Ensure last SPC count is always visible
            ...
            print(f"Error in dynamic_xticks -> {e}")
            return []
    ...
    def set_limits(self, lol, lsl, lcl, nominal, ucl, usl, uol, least_count):
        try:
            # Update SPC limit values
            self.limits = [lol, lsl, lcl, nominal, ucl, usl, uol]
            self.least_count = least_count

            self.update_limit_artists()
            # The limit update can change ylim, ticks, bands, and labels.
            # Refresh the cached background before the next blitted point.
            self.cache_background()
        except Exception as e:
    ...
    def reset_graph(self):
        try:
            # Clear graph data and redraw background
            self.data_raw.clear()
            self.line.set_data([], [])
            # self.canvas.draw()
            self.cache_background()
        except Exception as e:
            print(f"Error in reset_graph -> {e}")

    def clamp_value(self, value):
        try:
            # Restrict value within outer SPC limits
            lol, lsl, lcl, nominal, ucl, usl, uol = self.limits
    ...
    def generate_graph(self, raw_data):
        try:
            ...
            if not self.data_raw:
                self.line.set_data([], [])
                # self.ax.set_xticks(self.dynamic_xticks(0))
            else:
                count = len(self.data_raw)
                self.line.set_data(
                ...
                # self.ax.set_xticks(self.dynamic_xticks(count))
            # t = time.perf_counter()
            self.canvas.draw()
            # print("generate_graph",time.perf_counter() - t)
            self.background = self.canvas.copy_from_bbox(self.ax.bbox)
        except Exception as e:
    ...
    def set_margins(self, left, right, top, bottom):
        try:
            ...
            # Rebuild background and redraw existing blue line
            self.cache_background()

        except Exception as e:
            print(f"Error in set_margins -> {e}")
...
class SPCWidget(QWidget):
    def __init__(self, spc: SPCBase, parent=None):
        try:
            ...
            print("Error in SPCWIdget __init__ ->",e)
            
    def load_value(self, value):
        try:
            # Update SPC graph with latest value
    ...
    def resizeEvent(self, event):
        try:
            # Delay background caching until resize operation completes
...
class SPCManager:
    def __init__(self, main_window):
        try:
            ...
            self.worker = None
            self.pending_program_id = None

            self.last_applied_count = 0    # ← track last layout build
    ...
    def ensure_layout(self, widget):
        try:
            # Create layout if widget does not already have one
            if widget.layout() is None:
                layout = QVBoxLayout(widget)
                layout.setContentsMargins(0, 0, 0, 0)
                widget.setLayout(layout)
        except Exception as e:
            print(f"Error in ensure_layout -> {e}")
        ...
        try:
            ...
            if self.worker_thread is not None:
                if self.worker_thread.isRunning():
                    self.pending_program_id = program_id
            ...
            self.pending_program_id = None
            self.start_loader_thread(program_id)

        except Exception as e:
            print(f"load_program_OnSpc error: {e}")
 
    def start_loader_thread(self, program_id: int):
        try:
            ...
            self.worker.finished.connect(self.on_limits_loaded)
            ...
            self.worker_thread.finished.connect(self.run_pending_if_any)
            ...
            print(f"start_loader_thread error: {e}")
 
    def run_pending_if_any(self):
        try:
            # print("SPCLoader thread finished")

            self.worker_thread = None

            if self.pending_program_id is not None:
                pid = self.pending_program_id
                self.pending_program_id = None

                print(f"SPCLoader running queued program_id={pid}")

                self.start_loader_thread(pid)

        except Exception as e:
            print(f"run_pending_if_any error: {e}")
            
    def on_limits_loaded(self, results):
        try:
            ...
            for item in results:
                ...
                limits = [
                    item["lol"], item["lsl"], item["lcl"],
                    item["nominal"], item["ucl"], item["usl"], item["uol"],
                ]
                title = f"{item['dim']} {item['dim_name']}"

                # set_limits redraws the full Matplotlib canvas.  Avoid a
                # redraw for channels whose limits did not change.
                if (
                    spc_widget.spc.limits != limits
                    or spc_widget.spc.least_count != item["least_count"]
                ):
                    spc_widget.spc.set_limits(*limits, item["least_count"])

                if spc_widget.spc.title != title:
                    spc_widget.spc.set_title(title)
                    spc_widget.spc.canvas.draw_idle()
            current_program = int(
                self.ui.valueObj.activeVariables_dict["ActiveProgramId"]
            )

            if self.loaded_program_id != current_program:
                self.load_data(channels)
                self.loaded_program_id = current_program
        ...
        try:
            ...
            for dim in self.active_channels:
                ...
                if probe_id:
                    ...
                    spi_controller = getattr(self.ui, "spi_controller", None)
                    if spi_controller is not None:
                        spi_controller.job_count[probe_id] = 1
                    self.ui.data_handler.show_job_count_on_dimension_labels(
                        [(f"D{dim}", 0)]
                    )
                    reset_dims.append(f"D{dim}")
            for ch in self.active_channels:
                spc_widget = self.spc_all.get(ch)
                if spc_widget: 
                    spc_widget.reset()
                if reset_dims:
                        msg = f"SPC reset completed for {', '.join(reset_dims)}"
    ...
    def apply_layout(self):
        ...
        try:
            ...
            if not self.layout_initialized or count != self.last_applied_count:
            ...
            self.apply_markersize(count)
        except Exception as e:
            print(f"Error while applying layout -> {e}")
            
    def apply_markersize(self, count):
        try:
            # Adjust graph marker size based on active channel count
            if count == 1:
                marker_size = 6
            elif count >= 2:
                marker_size = 3
            ...
            for ch in self.active_channels:
                spc = self.spc_all[ch].spc
                spc.line.set_markersize(marker_size)

        except Exception as e:
            print(f"Error in apply_markersize -> {e}")
...
class DialIndicator(QObject):
    def __init__(self, main_window):
        try:
            ...
            # Temporary live correction display
            self.correction_timers = {
                1: None,
                2: None,
                3: None,
                4: None,
            ...
            # if not self.correction_active[1]:
            #     self.ui.button_mode1.setText(
            #         self.mode_by_channel.get(1, self.ui.button_mode1.text())
            #     )

            # if not self.correction_active[2]:
            #     self.ui.button_mode2.setText(
            #         self.mode_by_channel.get(2, self.ui.button_mode2.text())
            #     )

            # if not self.correction_active[3]:
            #     self.ui.button_mode3.setText(
            #         self.mode_by_channel.get(3, self.ui.button_mode3.text())
            #     )

            # if not self.correction_active[4]:
            #     self.ui.button_mode4.setText(
            #         self.mode_by_channel.get(4, self.ui.button_mode4.text())
            #     )
            # self._pm = {
            ...
            self.geo = {
                ...
                2: {
                    ...
                    "label_D1name_Dial": QRect(0,  0, 81,  20),
                    
                    "val1_Digit":   QRect(40,   60, 320, 101),
                    "sts1_Digit":   QRect(25,  240, 270,  81),
                    "btn1_Digit":   QRect(305, 255,  81,  50),
                    "label_D1name_Digit": QRect(20,  0, 81,  20),
                    ...
                    "label_D2name_Dial": QRect(390,  0, 81,  20),

                    "val2_Digit":   QRect(420,  60, 320, 101),
                    "sts2_Digit":   QRect(403, 240, 270,  81),
                    "btn2_Digit":   QRect(680, 255,  81,  50),
                    "label_D2name_Digit": QRect(420,  0, 81,  20),
                    "pm_dial": 380,
                },
                3: {
                    "dial1_Dial":  QRect(60,  -5, 270, 145),
                    "val1_Dial":   QRect(0,  134, 175,  42),
                    "sts1_Dial":   QRect(178,134, 120,  42),
                    "btn1_Dial":   QRect(305,134,  70,  42),
                    "label_D1name_Dial": QRect(0,  0, 81,  20),
                    
                    "val1_Digit":  QRect(40,  45, 270,  80),
                    "sts1_Digit":  QRect(50, 133, 240,  42),
                    "btn1_Digit":  QRect(314,133,  70,  42),
                    "label_D1name_Digit": QRect(20,  0, 81,  20),
                    
                    "dial2_Dial":  QRect(455, -5, 270, 145),
                    "val2_Dial":   QRect(390,134, 175,  42),
                    "sts2_Dial":   QRect(568,134, 120,  42),
                    "btn2_Dial":   QRect(690,134,  70,  42),
                    "label_D2name_Dial": QRect(390,  0, 81,  20),
                    
                    "val2_Digit":  QRect(450, 45, 270,  80),
                    "sts2_Digit":  QRect(430,133, 240,  42),
                    "btn2_Digit":  QRect(680,133,  70,  42),
                    "label_D2name_Digit": QRect(420,  0, 81,  20),

                    "dial3_Dial":  QRect(250,172, 320, 145),
                    "val3_Dial":   QRect(180,310, 175,  42),
                    "sts3_Dial":   QRect(360,310, 130,  42),
                    "btn3_Dial":   QRect(500,310,  70,  42),
                    "label_D3name_Dial": QRect(200,  190, 81,  20),
                    
                    "val3_Digit":  QRect(250,225, 320,  80),
                    "sts3_Digit":  QRect(240,310, 240,  42),#260
                    "btn3_Digit":  QRect(500,310,  70,  42),#520
                    "label_D3name_Digit": QRect(220,  180, 81,  20),
                    "pm_dial": 260,
                },
                4: {
                    "dial1_Dial":  QRect(60,  -5, 270, 140),
                    "val1_Dial":   QRect(0,  133, 171,  41),
                    "sts1_Dial":   QRect(180,133, 111,  41),
                    "btn1_Dial":   QRect(305,133,  70,  41),
                    "label_D1name_Dial": QRect(0,  0, 81,  20),
                    
                    "val1_Digit":  QRect(40,  40, 260,  70),
                    "sts1_Digit":  QRect(30, 120, 250,  48),
                    "btn1_Digit":  QRect(298,120,  70,  48),
                    "label_D1name_Digit": QRect(10,  0, 81,  20),
                    ...
                    "val2_Digit":  QRect(440, 40, 260,  70),
                    "sts2_Digit":  QRect(430,120, 250,  48),
                    "btn2_Digit":  QRect(690,120,  70,  48),
                    "label_D2name_Digit": QRect(400,  0, 81,  20),

                    
                    "dial3_Dial":  QRect(60, 170, 270, 140),
                    "val3_Dial":   QRect(0,  311, 171,  41),
                    "sts3_Dial":   QRect(180,311, 111,  41),
                    "btn3_Dial":   QRect(305,311,  70,  41),
                    "label_D3name_Dial": QRect(0,  180, 81,  20),
                    
                    "val3_Digit":  QRect(40, 220, 260,  70),
                    "sts3_Digit":  QRect(30, 300, 250,  48),
                    "btn3_Digit":  QRect(298,300,  70,  48),
                    "label_D3name_Digit": QRect(10,  180, 81,  20),
                    ...
                    "label_D4name_Dial": QRect(390,  180, 81,  20),
                    
                    "val4_Digit":  QRect(440,220, 260,  70),
                    "sts4_Digit":  QRect(430,300, 250,  48),
                    "btn4_Digit":  QRect(690,300,  70,  48),
                    "label_D4name_Digit": QRect(400,  180, 81,  20),
            ...
            # self.app.dimension_delete.connect(self.set_visibility)
        except Exception as e:
            print(f"Error in DialIndicator.__init__: {e}")

    # ── mode toggle helpers ───────────────────────────────────────────────────
    # FIX 1: toggle_mode does NOT call set_active_channels / apply_layout.
    ...
    def toggle_mode(self, dial_num: int):
        ...
        try:
            ...
            # Cancel temporary correction display.
            # --------------------------------------------------
            timer = self.correction_timers.get(dial_num)

            if timer is not None:
                timer.stop()
                timer.deleteLater()
                self.correction_timers[dial_num] = None

            self.correction_active[dial_num] = False

            current = self.mode_by_channel.get(
                dial_num,
                btn.text()
            )

            current = self.mode_by_channel.get(dial_num, btn.text())
            ...
            self.mode_by_channel[dial_num] = new_mode

            # ── pure geometry update — no draw calls ──────────────────────
            self.update_single_dial(dial_num)
            ...
            if new_mode == 'SPC' and self.spc_manager:
                ...
                if spc and self.channel_count > 0:
                    self.apply_spc_geometry(dial_num, spc)

        except Exception as e:
            print(f"Error in toggle_mode({dial_num}): {e}")

    def apply_spc_geometry(self, dial_num: int, spc_widget):
        """
        ...
        geo_map = {
            1: {"1": (QRect(0, 0, 771, 200), 150, (0.10, 0.95, 0.90, 0.20))},
            2: {
            ...
            3: {
                ...
                "3": (QRect(42, 179, 600, 130),120, (0.20, 0.95, 0.90, 0.26)),
            },
            4: {
                "1": (QRect(5, 0, 370, 130),    120, (0.20, 0.95, 0.90, 0.30)),
                "2": (QRect(383, 0, 370, 130),  120, (0.20, 0.95, 0.90, 0.30)),
                "3": (QRect(5, 180, 370, 130),  120, (0.20, 0.95, 0.90, 0.30)),
                "4": (QRect(383, 180, 370, 130),120, (0.20, 0.95, 0.90, 0.30)),
            },
        }
        ...
        # Keep the value, status, and mode controls aligned with the SPC slot
        # when a save refreshes visibility without changing the mode tuple.
        g = self.geo.get(count, {})
        label_value = getattr(self.ui, f"label_value{dial_num}")
        label_status = getattr(self.ui, f"label_status{dial_num}")
        button_mode = getattr(self.ui, f"button_mode{dial_num}")
        label_value.setGeometry(g[f"val{dial_num}_Dial"])
        label_status.setGeometry(g[f"sts{dial_num}_Dial"])
        button_mode.setGeometry(g[f"btn{dial_num}_Dial"])

        if margins:
            spc.set_margins(*margins)     # one draw_idle() for this one chart

    # ── mode button slots ─────────────────────────────────────────────────────
    def set_dimension_1_ui(self): self.toggle_mode(1)
    def set_dimension_2_ui(self): self.toggle_mode(2)
    def set_dimension_3_ui(self): self.toggle_mode(3)
    def set_dimension_4_ui(self): self.toggle_mode(4)

    # ── single-dial geometry update (no draw calls) ───────────────────────────
    def update_single_dial(self, dial_num: int):
        try:
            if self.channel_count == 0: return
            g = self.geo.get(self.channel_count)
            if not g or f"dial{dial_num}_Dial" not in g: return

            mode      = self.mode_by_channel.get(dial_num, 'Dial')
            anim_attr = f"dial_{dial_num}_anim"

            widgets = {
                1: (self.ui.label_dial1, self.ui.label_needle1,
                    self.ui.label_value1, self.ui.label_status1,
                    self.ui.button_mode1, self.ui.widget_spc1,self.ui.label_D1name),
                2: (self.ui.label_dial2, self.ui.label_needle2,
                    self.ui.label_value2, self.ui.label_status2,
                    self.ui.button_mode2, self.ui.widget_spc2,self.ui.label_D2name),
                3: (self.ui.label_dial3, self.ui.label_needle3,
                    self.ui.label_value3, self.ui.label_status3,
                    self.ui.button_mode3, self.ui.widget_spc3,self.ui.label_D3name),
                4: (self.ui.label_dial4, self.ui.label_needle4,
                    self.ui.label_value4, self.ui.label_status4,
                    self.ui.button_mode4, self.ui.widget_spc4,self.ui.label_D4name),
            }
            label_dial, label_needle, label_value, label_status, btn_mode, widget_spc,label_Dname = \
                widgets[dial_num]

            dial_font  = {1:"40pt",2:"33pt",3:"17pt",4:"17pt"}.get(self.channel_count,"17pt")
            digit_font = {1:"55pt",2:"42pt",3:"35pt",4:"22pt"}.get(self.channel_count,"25pt")
            ...
            if mode == 'Dial':
                ...
                label_Dname.setGeometry(g[f"label_D{dial_num}name_Dial"])
                label_Dname.setAlignment(Qt.AlignmentFlag.AlignCenter)
                ...
                # label_Dname.setStylesheet(self.style_status + f"font-size: {dial_font};")
                label_Dname.setStyleSheet(u"border: 1px solid #888888;font-size: 10pt;color: white;")
                ...
                label_Dname.setGeometry(g[f"label_D{dial_num}name_Digit"])
                label_Dname.setAlignment(Qt.AlignmentFlag.AlignCenter)
                ...
                # label_Dname.setStylesheet(self.style_status + f"font-size: {digit_font};")
            elif mode == 'SPC':
                label_value.setGeometry(g[f"val{dial_num}_Dial"])
                label_value.setAlignment(Qt.AlignmentFlag.AlignCenter)
                label_status.setGeometry(g[f"sts{dial_num}_Dial"])
            ...
            print(f"Error in update_single_dial({dial_num}): {e}")

    def destroy_dial_handlers(self):
    ...
    def get_font_size(self, dial_num: int) -> str:
        mode = self.mode_by_channel.get(dial_num, 'Dial')
        if mode == 'Digit':
            return {1:"55pt",2:"42pt",3:"35pt",4:"35pt"}.get(self.channel_count,"25pt")
        return {1:"40pt",2:"33pt",3:"17pt",4:"17pt"}.get(self.channel_count,"17pt")
    def show_live_correction(self, dimension, correction):
        """
        Show live AOC correction on button_mode1..4 for 3 seconds.

        IMPORTANT:
        mode_by_channel is NOT changed.
        """

        try:
            dimension = str(dimension).strip().upper()
            ...
            if dimension.startswith("D"):
                dial_num = int(dimension[1:])
            else:
                dial_num = int(dimension)

            if dial_num not in (1, 2, 3, 4):
                print(
                    f"Invalid dimension for live correction: {dimension}"
                )
                return

            button = getattr(
                self.ui,
                f"button_mode{dial_num}"
            )

            # Stop old timer if correction comes again
            old_timer = self.correction_timers.get(dial_num)

            if old_timer is not None:
                old_timer.stop()
                old_timer.deleteLater()

            self.correction_active[dial_num] = True

            # IMPORTANT:
            # Do NOT change self.mode_by_channel
            button.setText(
                f"{float(correction):+.3f}"
            )
            ...
            timer.timeout.connect(
                lambda d=dial_num:
                    self.restore_mode_button(d)
            )

            self.correction_timers[dial_num] = timer

            timer.start(4000)

            # print(
            #     f"D{dial_num}: Live correction "
            #     f"{float(correction):+.3f}"
            # )

        except Exception as e:
            print(
                f"Error in show_live_correction -> {e}"
            )
    def restore_mode_button(self, dial_num):
        """
        Restore the button to its actual Dial/SPC/Digit state.
        """
        ...
        try:
            ...
            # mode_by_channel is the real source of truth
            current_mode = self.mode_by_channel.get(
                dial_num,
                "Dial"
            )

            button.setText(current_mode)
            ...
            # print(
            #     f"D{dial_num}: restored -> {current_mode}"
            # )

        except Exception as e:
            print(
                f"Error in restore_mode_button -> {e}"
            )
    def set_visibility(self):
        ...
        """
        try:
            self.ui.button_mode1.setText(self.mode_by_channel.get(1, self.ui.button_mode1.text()))
            self.ui.button_mode2.setText(self.mode_by_channel.get(2, self.ui.button_mode2.text()))
            self.ui.button_mode3.setText(self.mode_by_channel.get(3, self.ui.button_mode3.text()))
            self.ui.button_mode4.setText(self.mode_by_channel.get(4, self.ui.button_mode4.text()))

            for i in range(1, 5):
                mode = self.mode_by_channel.get(i, 'Dial')
                if mode != 'SPC':
                    getattr(self.ui, f"widget_spc{i}").hide()
                    getattr(self.ui, f"label_D{i}name").show()
            ...
            for i in range(self.channel_count + 1, 5):
                getattr(self.ui, f"widget_spc{i}").hide()
                getattr(self.ui, f"label_D{i}name").hide()
            # Always update dial ranges
            
            if self.channel_count != self.last_channel_count:
                # print(f"Channel count {self.last_channel_count}→{self.channel_count}, rebuilding")
                self.destroy_dial_handlers()
                self.last_channel_count = self.channel_count
            ...
            if self.spc_manager:
                ...
                for i in range(1, self.channel_count + 1):
                    if self.mode_by_channel.get(i, 'Dial') == 'SPC':
                        spc = self.spc_manager.spc_all.get(i)
                        if spc:
                            self.apply_spc_geometry(i, spc)

            # ── STATE GUARD ───────────────────────────────────────────────
            new_state = (
                self.channel_count,
                self.mode_by_channel.get(1),
                self.mode_by_channel.get(2),
                self.mode_by_channel.get(3),
                self.mode_by_channel.get(4),
            )
            for n in range(1, self.channel_count + 1):
                anim_attr = f"dial_{n}_anim"
                if hasattr(self.ui, anim_attr):
                    probe = f"D{n}"
                    probe_data = self.ui.valueObj.ProgramSettings_dict.get(probe, {})
                    probe_settings = probe_data.get("ProbeBasedSettings", {})
                    dial_range = probe_settings.get("Range", 0.3) 
                    getattr(self.ui, anim_attr).set_dial_range(dial_range)
            if new_state == self.last_visibility_state:
                return
            self.last_visibility_state =new_state
            ...
            for n in range(1, self.channel_count + 1):
                ...
                anim_attr = f"dial_{n}_anim"
                mode      = self.mode_by_channel.get(n, 'Dial')
                ...
                if not hasattr(self.ui, anim_attr):
                    setattr(self.ui, anim_attr, DialHandler(
                        ...
                        dial   = f"/home/torizon/app/src/images/dial-30-yellow-{cfg['dial_img']}.png",
                        needle = cfg["svg"],
                        probe  = f"D{n}",
