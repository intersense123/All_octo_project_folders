...
class SetMasterController(QObject):
    ...
    def clear_failed_master_cache(self, probes):
        try:
            controller = getattr(self.ui_parent, "controller", None)
            cache_signal = getattr(controller, "dimension_cache_status_deleted", None)
            if cache_signal is None:
                return

            for probe in dict.fromkeys(probes):
                cache_signal.emit(probe)
            active_program_id = int(
                self.ui_parent.valueObj.activeVariables_dict["ActiveProgramId"]
            )
            with SessionLocal() as session:
                settings = session.query(ProbeBasedSettings).filter(
                    ProbeBasedSettings.ProgramId == active_program_id,
                    ProbeBasedSettings.Dimension.in_(list(dict.fromkeys(probes)))
                ).all()

                for probe_settings in settings:
                    row = session.query(SetMasterSettings).filter(
                        SetMasterSettings.ProgramId == active_program_id,
                        SetMasterSettings.Dimension == probe_settings.Dimension
                    ).first()
                    if row is None:
                        continue

                    row.MasterValue = None
                    row.RemovedPartValue = None
                    row.SPI_master = None

                    if probe_settings.MasterType == "Single Master":
                        # Keep the factory LOW/HIGH calibration intact.
                        continue
            ...
            print(f"Error clearing failed master database values: {e}")
    ...
    def load_single_master_calibration_db(self):
        try:
            with SessionLocal() as session:
                rows = session.query(SingleMasterCalibration).all()
                if not rows:
                    print("[SINGLE MASTER] No saved factory calibration found")
                    return False

                loaded_any = False
                for row in rows:
                    probe = self.normalize_calibration_probe(row.Probe)

                    if row.ConvertedRawValue is not None:
                        self.single_master_converted_raw_value[probe] = row.ConvertedRawValue
    ...
    def get_saved_calibration(self, physical_probe):
        physical_probe = self.normalize_calibration_probe(physical_probe)
        cached = self.single_master_converted_raw_value.get(physical_probe)
        if cached is not None:
            return cached
        with SessionLocal() as session:
            row = session.query(SingleMasterCalibration).filter(
                SingleMasterCalibration.Probe == physical_probe
            ).first()
            if row and row.ConvertedRawValue is not None:
                self.single_master_converted_raw_value[physical_probe] = row.ConvertedRawValue
                return row.ConvertedRawValue
        return None
    ...
    def run(self):
            ...
            try:
                ...
                # Grouping is a user-selection convenience, not permission to
                # calibrate every selected channel with the same master type.
                # Keep this set so the single-master workflow can exclude
                # selected Double Master/No Master dimensions.
                single_master_dimensions = {
                    configured_row.Dimension
                    for configured_row in rows
                    if configured_row.MasterType == "Single Master"
                }
                ...
                for row in rows:
                    ...
                    if row.MasterType == "Single Master":
                        ...
                        # ----------------------------------------
                        if current == '1':
                            # For example, when D1 is Single Master and D2 is
                            # Double Master, selecting D1+D2 must only show D1
                            # in these single-master dialogs and must not write
                            # a single-master calibration for D2.
                            selected_probes = [
                                channel
                                for channel in self.grouped_channels
                                if channel in single_master_dimensions
                            ]
                        ...
                        for p in selected_probes:
                            ...
                            if low_spi is None:
                                low_spi = self.single_master_low_spi.get(probe)
    
                            if high_spi is None:
                                high_spi = self.single_master_high_spi.get(probe)
                        ...
                        if not ok:
                            ...
                            self.clear_failed_master_cache(selected_probes)
                            return
                        # ----------------------------------------
                        # CURRENT ROW VALUES
                        # ----------------------------------------
                        ...
                        # A No Master dimension does not take part in the
                        # single-master calibration, but it must be present in
                        # probe_data before master_finished is emitted.  This
                        # lets a formula such as D4 -> D1 render D4 immediately.
                        if success:
                            success, error_msg = self.register_no_master_dimensions()
                ...
                if current == '0':
                        ...
                        if not ok:
                            ...
                            self.clear_failed_master_cache(Active_channels)
                            return
                        ...
                        if not ok:
                            ...
                            self.clear_failed_master_cache(self.grouped_channels)
                            return
    ...
    def calculate_single_master(self, row):
        try:
            probe = row.Dimension
            calibration_probe = self.get_calibration_probe(row)   # e.g. "P1" — the ONLY key used

            spi_low  = self.single_master_low_spi.get(calibration_probe)
            spi_high = self.single_master_high_spi.get(calibration_probe)
            DB_low   = self.single_master_low_value.get(calibration_probe)
            DB_high  = self.single_master_high_value.get(calibration_probe)

            spi_master = self.single_master_masterSpi.get(probe)  # this stays dimension-based (live run-time pick)

            if spi_low is None or spi_high is None:
                return False, "LOW/HIGH SPI missing"
            if DB_low is None or DB_high is None:
                return False, "LOW/HIGH DB value missing"
            if spi_master is None:
                return False, "MASTER SPI missing"

            converted_raw_value = self.get_saved_calibration(calibration_probe)

            if converted_raw_value is None:
                raw_range = spi_high - spi_low
                if raw_range == 0:
                    return False, "RAW RANGE ZERO"
                converted_raw_value = (DB_high - DB_low) / raw_range
                ps = self.ui_parent.valueObj.ProgramSettings_dict.get('ProgramSpecificSettings', {})
                if ps.get("Uom") == 'inch':
                    converted_raw_value *= 25.4
                self.save_single_master_runtime_calibration(calibration_probe, converted_raw_value)
            ...
            self.probe_data[probe] = {
                "DB_low": DB_low, "DB_high": DB_high, "DB_master": row.Master,
                "method": row.Method,
                "USL": row.UpperSpecificationLimit, "LSL": row.LowerSpecificationLimit,
                "Master Type": row.MasterType,
                "Range": row.Range, "Nominal": row.NominalValue, "LeastCount": row.LeastCount,
                "SPI_low": spi_low, "SPI_high": spi_high, "SPI_master": spi_master,
    ...
    def register_no_master_dimensions(self):
        """Load display/status data for No Master dimensions in this program."""
        try:
            active_program_id = int(
                self.ui_parent.valueObj.activeVariables_dict["ActiveProgramId"]
            )
            ...
            with SessionLocal() as session:
                rows = session.query(ProbeBasedSettings).filter(
                    ProbeBasedSettings.ProgramId == active_program_id,
                    ProbeBasedSettings.MasterType == "No Master"
                ).all()
            ...
            print(f"[register_no_master_dimensions ERROR] {e}")
            return False, str(e)


    def save_single_master_runtime_calibration(self, physical_probe, converted_raw_value):
        try:
            with SessionLocal() as session:
                row = session.query(SingleMasterCalibration).filter(
                    SingleMasterCalibration.Probe == physical_probe
                ...
                if row is None:
                    row = SingleMasterCalibration(Probe=physical_probe)
                    session.add(row)
                row.ConvertedRawValue = converted_raw_value
                session.commit()
            self.single_master_converted_raw_value[physical_probe] = converted_raw_value
        except Exception as e:
...
class SPIController(QObject):
    ...
    def __init__(self, main_window, data_handler,app_controller):
        try:
            ...
            self.prev_status = {}
            # A completed part stays locked until its probe returns to the
            # removed-part/autosense reading.
            self.frozen_measurements = {}
            self.probe_in_autosense = {}
            self.relay_pending_for_cycle = False
            # Tracks values saved manually or automatically in this frozen
            # fixture cycle; autosense clears the entry.
            self.saved_frozen_measurements = {}
    ...
    def dimension_cache_status_clear(self, dimension):
        try:
            self.probe_data.pop(dimension, None)
            self.removed_parts.pop(dimension, None)
            self.master_low.pop(dimension, None)
            self.master_high.pop(dimension, None)
            self.no_master_limits.pop(dimension, None)
            self.single_master_low_spi.pop(dimension, None)
            self.single_master_high_spi.pop(dimension, None)
            self.single_master_low_value.pop(dimension, None)
            self.single_master_high_value.pop(dimension, None)
            self.raw_spi_values.pop(dimension, None)
            self.spi_values.pop(dimension, None)
            self.least_count_map.pop(dimension, None)
            self.last_status.pop(dimension, None)
            self.last_raw_status.pop(dimension, None)
            self.status_stable_since.pop(dimension, None)
            self.previous_measurements.pop(dimension, None)
            self.current_measurements.pop(dimension, None)
            self.changed_probes.pop(dimension, None)

            if self.master_set is not None:
                self.master_set.probe_data.pop(dimension, None)
                self.master_set.removed_parts.pop(dimension, None)
                self.master_set.master_low.pop(dimension, None)
                self.master_set.master_high.pop(dimension, None)
                self.master_set.no_master_limits.pop(dimension, None)

            self.refresh_settings()

            # Do not restore the base 30pt style here.  It is only a fallback
            # style; in a three/four-channel layout it overflows the compact
            # value label after a failed AutoSense/master setup.
            try:
                probe_num = int(str(dimension).strip().upper().removeprefix("D"))
                font_size = self.ui.dial_indicator.get_font_size(probe_num)
            except (AttributeError, TypeError, ValueError):
                font_size = "17pt"

            status_label = self.status_labels.get(dimension)
            if status_label:
                status_label.clear()
            ...
            value_label = self.probe_labels.get(dimension)
            if value_label:
            ...
            # -------------------------------------------------
            # FAILED / CLEARED MASTER = YELLOW DIAL
            # -------------------------------------------------
            self.safe_set_dial_color(dimension, "yellow")
    ...
    def capture_single_master_spi(self, probe_label, mode):
        """probe_label = combobox text, e.g. 'P1'."""
        try:
            physical_probe = self.normalize_calibration_probe(probe_label)
            d_key = self.get_dimension_from_probe(probe_label)  # only used to read live spi
            if not d_key:
                return
            ...
            if mode == "low":
                self.single_master_low_spi[physical_probe] = spi
                self.single_master_low_value[physical_probe] = float(
                    self.ui.lineEdit_lowerMasterCalibrate.text()
                )
                self.ui.status_manager.show(
                    page_index=19,
                    message= f"Lower Calibration for {physical_probe} Completed",
                    timeout=5000,
                    msg_type="success"
                )

            elif mode == "high":
                self.single_master_high_spi[physical_probe] = spi
                self.single_master_high_value[physical_probe] = float(
                    self.ui.lineEdit_higherMasterCalibrate.text()
                )
                low_spi = self.single_master_low_spi.get(physical_probe)
                if low_spi is not None and low_spi == spi:
                    self.ui.status_manager.show(
                        page_index=19,
                        message=f"{physical_probe}: Lower and Higher SPI cannot be same",
                        timeout=5000,
                        msg_type="error"
                    )
                    return
                self.ui.status_manager.show(
                    page_index=19,
                    message= f"Higher Calibration for {physical_probe} Completed",
                    timeout=5000,
                    msg_type="success"
                )
                self.save_single_master_calibration_db(physical_probe)
        except Exception as e:
            print(f"[capture_single_master_spi ERROR] {e}")

    def save_single_master_calibration_db(self, physical_probe):
        try:
            spi_low = self.single_master_low_spi.get(physical_probe)
            spi_high = self.single_master_high_spi.get(physical_probe)
            db_low = self.single_master_low_value.get(physical_probe)
            db_high = self.single_master_high_value.get(physical_probe)
    ...
    def get_dimension_from_probe(self, probe):
        try:
            for i, item in enumerate(self.formula_Cache):
                ...
                if item == probe:
                    return f"D{i+1}"
                print(f"[get_dimension_from_probe] NO MATCH for probe='{probe}' in formula_Cache={self.formula_Cache}")
    ...
    def handle_save_signal(self, success):
        try:
            ...
            if state == '1':
                return
            if not success:
                # A footswitch press displays this feedback through
                # MainWindow. A frozen reading may correctly produce no new
                # database record, so do not hide that feedback here.
                return
    ...
    def is_reading_freeze_enabled(self):
        READING_FREEZEONOFF = (
            self.ui.valueObj.IOSettings_dict
            .get("READING_FREEZE", {})
            .get("Enable", "0")
        )
        return str(READING_FREEZEONOFF).strip() == "1"

    def reset_measurement_cycles(self):
        """Discard all frozen part readings after master/program changes."""
        self.frozen_measurements.clear()
        self.saved_frozen_measurements.clear()
        self.probe_in_autosense.clear()
        self.current_measurements = {}
        self.previous_measurements = {}
    ...
    def enter_autosense(self, probe):
        """Reset one completed part only on the autosense transition."""
        if not self.probe_in_autosense.get(probe, False):
            self.frozen_measurements.pop(probe, None)
            self.saved_frozen_measurements.pop(probe, None)
            self.previous_measurements.pop(probe, None)
            self.changed_probes.pop(probe, None)
            # Do not save or pulse a part which has already been removed.
            self.measurement_changed_since_save = False
            self.relay_pending_for_cycle = False

        self.probe_in_autosense[probe] = True
        self.last_status[probe] = ""
        self.last_raw_status[probe] = None
        self.status_stable_since[probe] = None

    def leave_autosense(self, probe):
        self.probe_in_autosense[probe] = False

    def freeze_measurement(self, probe, value, status):
        """Lock the first status-stable part reading until autosense."""
        self.frozen_measurements[probe] = (value, status)
    ...
    # Read probe sensitivities from the current program-settings cache.
    # The cache is refreshed before probe_setttings_saved reaches this handler.
    def get_all_probe_sensitivity(self):
        try:
            program_settings = self.ui.valueObj.ProgramSettings_dict or {}
            sensitivities = []

            for probe in ("D1", "D2", "D3", "D4"):
                probe_settings = program_settings.get(probe, {}).get(
                    "ProbeBasedSettings", {}
                )
                try:
                    sensitivities.append(
                        int(float(probe_settings.get("ProbeSensitivity") or 0))
                    )
    ...
    def get_formula_bar(self):
        try:
            program_settings = self.ui.valueObj.ProgramSettings_dict or {}
            cached_channels = self.app.get_channels_for_program(
                int(self.ui.valueObj.activeVariables_dict["ActiveProgramId"])
            )

            if program_settings:
                return [
                    program_settings.get(channel, {})
                    .get("ProbeBasedSettings", {})
                    .get("Formula")
                    for channel in cached_channels[:4]
                ]

            # Fallback for startup before the program-settings cache exists.
            formula_bar_text = []
        ...
        try:
            ...
            with SessionLocal() as session:
                for probe, data in self.probe_data.items():
                    ...
                    if data.get("Master Type") == "No Master":
    ...
    def load_saved_master_settings(self):
        try:
            ...
            with SessionLocal() as session:
                ...
                probe_rows = session.query(ProbeBasedSettings).filter(
                    ProbeBasedSettings.ProgramId == active_program_id,
                    ProbeBasedSettings.MasterType == "No Master"
                ).all()

                for probe_db in probe_rows:
                    self.no_master_limits[probe_db.Dimension] = {
                    ...
                    self.probe_data[probe_db.Dimension] = {
                        "Master Type": "No Master",
                        "method": probe_db.Method,
                        "USL": probe_db.UpperSpecificationLimit,
                        "LSL": probe_db.LowerSpecificationLimit,
                ...
                for row in rows:
                    ...
                    if master_type == "Single Master":
                        ...
                        # SingleMasterCalibration is a permanent factory
                        # calibration keyed by physical probe (P1–P4), not by
                        # program or SPI dimension.
    ...
    def load_least_counts(self):
        """
        Populate the least-count cache from current program settings.
        """
        ...
        try:
            ...
            for dimension, settings in program_settings.items():
                if not str(dimension).startswith("D"):
                    continue

                probe_settings = settings.get("ProbeBasedSettings", {})
                self.least_count_map[dimension] = float(
                    probe_settings.get("LeastCount") or 0
                )
        except Exception as e:
    ...
    def ok_notok_part_check(self):
        try:
            ...
            ps = self.ui.valueObj.ProgramSettings_dict.get("ProgramSpecificSettings", {})
            self.auto_save_duration = float(ps.get("AutoSave_Duration", 1.0))
            reading_freeze_enabled = self.is_reading_freeze_enabled()
            if not reading_freeze_enabled:
                # Do not revive a value that was frozen before this setting
                # was switched off. Normal live behavior starts immediately.
                self.frozen_measurements.clear()
                self.saved_frozen_measurements.clear()
                self.probe_in_autosense.clear()
                self.relay_pending_for_cycle = False
            ...
            for probe, pdata in self.probe_data.items():
                ...
                if pdata["Master Type"] == "No Master":
                    continue
            ...
            for probe, pdata in self.probe_data.items():
                ...
                in_autosense = (
                    autoSense_value is not None
                    and spi_new is not None
                    and abs(spi_new - autoSense_value) < AUTOSENSE_TOLERANCE_RANGE
                )
                if in_autosense:
                    if reading_freeze_enabled:
                        self.enter_autosense(probe)
                    else:
                        self.last_status[probe] = ""
                        self.last_raw_status[probe] = None
                        self.status_stable_since[probe] = None
                ...
                if reading_freeze_enabled:
                    self.leave_autosense(probe)
                frozen = (
                    self.frozen_measurements.get(probe)
                    if reading_freeze_enabled else None
                )
                if frozen is not None:
                    frozen_value, frozen_status = frozen
                    color = self.status_color(frozen_status)
                    self.safe_move_needle(
                        probe,
                        self.calculate_angle(
                            frozen_value, pdata["Nominal"], pdata["Range"], lc
                        )
                    )
                    if label:
                        label.setText(self.format_by_leastcount(frozen_value, lc))
                        label.setStyleSheet(
                            self.ui.dial_indicator.style_value +
                            f"color: {color};font-size: {font_size};"
                        )
                    if status_label:
                        status_label.setText(frozen_status)
                        status_label.setStyleSheet(
                            self.ui.dial_indicator.style_status +
                            f"color: {color};font-size: {font_size};"
                        )
                    self.safe_set_dial_color(probe, color)
                    self.current_measurements[probe] = frozen
                    continue
                ...
                if reading_freeze_enabled:
                    self.freeze_measurement(probe, final_value, status)
                self.current_measurements[probe] = (
                    final_value,
                    status
                )

            # No Master dimensions can be formula-derived (for example,
            # D4 -> D1). Process their status before the measurement, relay and
            # autosave state is evaluated so they follow the same stable-time
            # behaviour as Single/Double Master dimensions.
            self.Nomaster_label(calculated_values, current_time)
            ...
            if self.relay_state == '1' and self.last_status:
                ...
                if reading_freeze_enabled:
                    if measurement_changed:
                        self.last_trigger_time = current_time
                        self.relay_triggered_for_cycle = False
                        self.relay_pending_for_cycle = True

                    # Only the first status-stable value in a fixture cycle
                    # can pulse the relay. Autosense cancels an old cycle.
                    if (
                        self.relay_pending_for_cycle
                        and (current_time - self.last_trigger_time) >= self.auto_save_duration
                        and not self.relay_triggered_for_cycle
                    ):
                    ...
                    # Freeze off: preserve the original live-value relay flow.
                    if measurement_changed:
                        self.last_trigger_time = current_time
                        self.relay_triggered_for_cycle = False
                    if (current_time - self.last_trigger_time) >= self.auto_save_duration:
                        if not self.relay_triggered_for_cycle:
                            self.ui.relayController.trigger(
                                relay_color, self.relay_value
                            )
                            self.last_relay_color = relay_color
                            self.relay_triggered_for_cycle = True
    ...
    def get_autosense_source_probes(self, probe, visited=None):
        """Return calibrated dimensions that feed a calculated dimension.

        Autosense readings are captured per *dimension* during master setup,
        even if that dimension's formula reads a different physical probe.  A
        formula copy must therefore stop at a calibrated master dimension
        instead of following its formula again.  For example, with D1 = P3
        (Single Master) and D3 = D1 (No Master), D3 must compare D1's live and
        removed-part readings.  Expanding D1 to P3/D3 creates a false cycle
        and prevents autosense from being applied to D3.
        """
        try:
            ...
            # A saved removed-part value belongs to this calibrated master
            # dimension.  Do not expand the formula beyond it: the value in
            # ``spi_values[probe]`` is already formula-resolved and is the
            # same value used when the removed-part reading was captured.
            master_data = self.probe_data.get(probe, {})
            if (
                master_data.get("Master Type") != "No Master"
                and self.removed_parts.get(probe) is not None
            ):
                return [probe]

            formula_list = self.formula_Cache or []
            probe_index = int(probe[1:]) - 1
            if probe_index < 0 or probe_index >= len(formula_list):
                return [probe]

            formula = str(formula_list[probe_index] or "")
            references = re.findall(r"\b(?:D|P)([1-4])\b", formula.upper())
            if not references:
                return [probe]

            sources = []
            for reference in references:
                source_probe = f"D{reference}"
                if source_probe == probe:
                    sources.append(source_probe)
                else:
                    sources.extend(
                        self.get_autosense_source_probes(source_probe, visited)
                    )

            return list(dict.fromkeys(sources)) or [probe]
        except Exception as e:
            print(f"Error in get_autosense_source_probes: {e}")
            return [probe]

    def is_probe_in_autosense(self, probe, tolerance):
        for source_probe in self.get_autosense_source_probes(probe):
            auto_sense_value = self.removed_parts.get(source_probe)
            spi_value = self.spi_values.get(source_probe)
            if (
                auto_sense_value is not None
                and spi_value is not None
                and abs(spi_value - auto_sense_value) < tolerance
            ):
                return True
        return False

            
    def Nomaster_label(self, calculated_values, current_time=None):
        """Apply stable value/status handling to formula-derived No Master rows."""
        try:
            AUTOSENSE_TOLERANCE_RANGE = 20
            current_time = time.time() if current_time is None else current_time
            reading_freeze_enabled = self.is_reading_freeze_enabled()

            # Values restored at startup are stored on SPIController, while
            # values from a newly completed master setup are on master_set.
            # Merge both maps so one does not hide the other.
            no_master_limits = dict(self.no_master_limits)
            if self.master_set is not None:
                no_master_limits.update(self.master_set.no_master_limits)

            for probe, pdata in no_master_limits.items():
                label = self.probe_labels.get(probe)
                status_label = self.status_labels.get(probe)
                probe_num = int(probe[1:])
                font_size = self.ui.dial_indicator.get_font_size(probe_num)
                ...
                if raw_value is None or raw_value == 0:
                    ...
                    self.last_raw_status[probe] = None
                    self.status_stable_since[probe] = None
                    self.current_measurements.pop(probe, None)
                    continue

                least_count = self.get_lc_cached(probe)
                final_value = self.smash(raw_value, least_count)

                # Autosense clears a frozen formula-derived part too.
                if self.is_probe_in_autosense(probe, AUTOSENSE_TOLERANCE_RANGE):
                    if reading_freeze_enabled:
                        self.enter_autosense(probe)
                    else:
                        self.last_status[probe] = ""
                        ...
                        self.status_stable_since[probe] = None
                    if label:
                        label.setText(self.format_by_leastcount(final_value, least_count))
                        label.setStyleSheet(
                            self.ui.dial_indicator.style_value +
                            f"color: white;font-size: {font_size};"
                        )
                ...
                if frozen is not None:
                    ...
                    if label:
                        label.setText(
                            self.format_by_leastcount(frozen_value, least_count)
                        )
                        label.setStyleSheet(
                            self.ui.dial_indicator.style_value +
                            f"color: {color};font-size: {font_size};"
                ...
                # Do not show a status or its colour until this No Master
                # result has remained unchanged for the configured delay.
                if self.last_raw_status.get(probe) != raw_status:
                    self.last_raw_status[probe] = raw_status
                    self.status_stable_since[probe] = current_time
                    if status_label:
                ...
                # Include No Master measurements in autosave, relay and change
                # detection during the same SPI update as other master types.
                if reading_freeze_enabled:
                    self.freeze_measurement(probe, final_value, status)
                self.current_measurements[probe] = (final_value, status)
        except Exception as e:
    ...
    def auto_save_probe_values(self):
        ...
        try:
            ...
            for dim in channels:
                ...
                frozen_for_dim = None
                if reading_freeze_enabled:
                    frozen_for_dim = self.frozen_measurements.get(dim)
                    if (
                        frozen_for_dim is None
                        or self.saved_frozen_measurements.get(dim) == frozen_for_dim
                    ):
                        continue
                    frozen_value, frozen_status = frozen_for_dim
                    value_text = self.format_by_leastcount(
                        frozen_value, self.get_lc_cached(dim)
                    )
                ...
                if not probe_id:
                    continue
                if reading_freeze_enabled:
                    frozen_measurements_to_mark[dim] = frozen_for_dim
            ...
            if records:
                ...
                if reading_freeze_enabled:
                    self.saved_frozen_measurements.update(
                        frozen_measurements_to_mark
                    )
