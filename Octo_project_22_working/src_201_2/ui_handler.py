...
class UIHandler(QObject):
    ...
    def __init__(self, main_window):
        try:
            ...
            self.apply_ProbeBasedSettings_state() ###tmp
            ...
            ####Temp Disabled This labels Becuse we dosnt Use####
            self.ui.comboBox_ovalityOnOff.setDisabled(True)
            self.ui.label_ovality.setDisabled(True)
            self.ui.label_airSensitivityQuotient.setDisabled(True)
            self.ui.lineEdit_airSensitivityQuotient.setDisabled(True)
            ######temparory disable buttons for testing
            # self.toggleButton_readingFreeze.setDisabled(True)
            # self.toggelButton_cycleStopTimer.setDisabled(True)
            # self.toggelButton_partTraceability.setDisabled(True)
            # self.toggleButtontimeToSetMaster.setDisabled(True)
            self.ui.button_databaseSettings.setDisabled(True)
            self.ui.button_wifiSettings.setDisabled(True)
            self.ui.button_autosenseRange.setDisabled(True)
            #####################################################
        except Exception as e:
            print(f"Error in initializtion of ui_handler : {e}")
    #temp########################
    def set_widget_opacity(widget, opacity: float):
        """Set opacity — reuse existing effect."""
        effect = widget.graphicsEffect()

        if effect is None or not isinstance(
            effect,
    ...
    def apply_ProbeBasedSettings_state(self):
        try:
            ...
            for w in widgets:
                w.hide() if is_off else w.hide() ###tmp hide  w.show()
    ...
    def linkage_of_ui(self):
        ...
        #     self.ui.button_back.clicked.connect(lambda checked=False: (
        #         self.linkage_of_backButton(),
        #         self.handle_aocwidget_backButton()
        #     )
        # )
            
        #     self.ui.button_forward.clicked.connect(lambda checked = False :(
        ...
        #     )
            ...
            self.ui.button_partSettings.clicked.connect(self.open_part_settings)
    ...
    def toggle_button_eventHandling(self):
        try:
            ...
            self.ui.toggleButton_readingFreeze,
    ...
    def setup_lineedit_keyboards(self):
        try:
            ...
            numeric_lineedits = [
                ...
                self.ui.lineEdit_serialNo,self.ui.lineEdit_shift1FromTime,self.ui.lineEdit_shift1ToTime,
                self.ui.lineEdit_shift2FromTime,self.ui.lineEdit_shift2ToTime,self.ui.lineEdit_shift3FromTime,
                self.ui.lineEdit_shift3ToTime
            ]
    ...
    def clear_relay_time_focus(self):
        """Remove cursor focus from Relay Time after a touch or dialog action."""
        try:
            relay_time = self.ui.lineEdit_relayTime
            relay_time.deselect()
            relay_time.clearFocus()
            self.ui.page8.setFocus(Qt.OtherFocusReason)
        except Exception as e:
            print(f"Error clearing relay-time focus: {e}")
    ...
    def eventFilter(self, obj, event):
        try:
            # A dialog may restore focus to the field that was active before
            # it opened. Relay Time is a numpad-only field, so immediately
            # return its focus to the I/O settings page.
            if (
                obj == self.ui.lineEdit_relayTime
                and event.type() == QtCore.QEvent.FocusIn
            ):
                QTimer.singleShot(0, self.clear_relay_time_focus)
            ...
            if obj == self.ui.toggleButton_readingFreeze and event.type() == event.MouseButtonPress:
                current = self.ui.valueObj.IOSettings_dict.get('READING_FREEZE', {}).get('Enable', '0')
                # Flip the value
                self.ui.valueObj.IOSettings_dict['READING_FREEZE']['Enable'] = '0' if current == '1' else '1'
                # Update the pixmap and other widgets
                update_IOSetting_ReadingFreeze_toggle(self.ui)
                return True  # Event handled
    ...
    # Show the hidden password entry overlay when the logo is long clicked
...
class ShutdownMenu(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.btn_reboot.clicked.connect(self.restart)
    ...
    def handle_restart(self):
        """
        Called when the reboot button is clicked.
