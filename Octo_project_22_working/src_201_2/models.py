from sqlalchemy import Column, Integer, Sequence, String, Time, create_engine, Float, inspect, text, UniqueConstraint
...
# -----------------------------
# IO settings: READING_FREEZE, relay, timers,
...
class IOSettings(Base):
    ...
    def insert_defaults(cls, session):
        ...
        defaults = [
            {"Key": "READING_FREEZE", "Enable": "0", "Value": None},
...
# class SingleMasterCalibration(Base):
#     __tablename__ = "SingleMasterCalibration"
#     __table_args__ = (
#         UniqueConstraint("ProgramId", "Probe", "Dimension", name="uq_single_master_calibration"),
#     )

#     id = Column(Integer, primary_key=True, autoincrement=True)
#     ProgramId = Column(Integer, nullable=False)
#     Probe = Column(String, nullable=False)
#     Dimension = Column(String, nullable=False)
#     ConvertedRawValue = Column(Float, nullable=False)
class SingleMasterCalibration(Base):
    __tablename__ = "SingleMasterCalibration"

    Probe = Column(String, primary_key=True)   # "P1","P2","P3","P4" — hardware identity only
    SPI_low = Column(Float, nullable=True)
    SPI_high = Column(Float, nullable=True)
    DB_low = Column(Float, nullable=True)
    DB_high = Column(Float, nullable=True)
    ConvertedRawValue = Column(Float, nullable=True)
...
#         """Insert default IO settings if they don't exist"""
#         defaults = [
#             {"Key": "READING_FREEZE", "Enable": "0", "Value": "Ok"},
#             {"Key": "RELAY", "Enable": "0", "Value": "2"},
#             {"Key": "CYCLE_STOP_TIMER", "Enable": "0", "Value": "2"},
#             {"Key": "AUTO_SAVE_READING", "Enable": "0", "Value": None},
#             {"Key": "PART_TRACEABILITY", "Enable": "0", "Value": "Manual Reset"},
