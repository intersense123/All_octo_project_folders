...
def set_widget_opacity(widget, opacity: float):
    """Set opacity — reuse existing effect to avoid repeated compositor setup."""
...
def update_angle_toggle(win):
    ...
    if state == 'ON':
        win.toggelButton_angleCalculation.setPixmap(win.toggle_on_pix)
        for w in widgets:
            w.setEnabled(True)
            # set_widget_opacity(w, 1.0)
            #  
    else:
        win.toggelButton_angleCalculation.setPixmap(win.toggle_off_pix)
        for w in widgets:
            w.setDisabled(True)
            # set_widget_opacity(w, 0.4)
            #    
...
def update_rs232_toggle(win):
    ...
    # Update toggle pixmap
    if state == 'ON':
        win.toggleButton_rs232.setPixmap(win.toggle_on_pix)
        for w in widgets:
            w.setEnabled(True)
            # set_widget_opacity(w, 1.0)
        ...
        win.toggleButton_rs232.setPixmap(win.toggle_off_pix)
        for w in widgets:
            w.setDisabled(True)
            # set_widget_opacity(w, 0.4)
...
def update_autooffcet_toggle(win):
    ...
    is_off = win.radioButton_aocEnableOFF.isChecked()
    win.valueObj.activeVariables_dict["AOCOnOff"] = state
    widgets = [
        win.button_cncListAoc,
        win.button_reportsAoc,
        win.lineEdit_spcDataCount,win.label_spcDataCount,
        win.label_aocEnableONOFF,win.radioButton_aocEnableOn,win.radioButton_aocEnableOFF,
        ...
        win.label_workInProcess,win.lineEdit_workInProcess,
        win.label_skipOffsetCount,win.lineEdit_skipOffsetCount
    ]
    DimensionWise_Enable_Disabled_widgets = [
                    win.label_axis,win.comboBox_axis,
                    win.label_offsetNo,win.lineEdit_offsetNo,
                    win.label_upperOffsetLimit,win.lineEdit_upperOffcetLimit,
                    win.label_lowerOffsetLimit,win.lineEdit_lowerOffsetLimit,
    ...
    if state == 'ON' :
        win.toggleButton_aoc.setPixmap(win.toggle_on_pix)
        for w in widgets:
            w.setEnabled(True)
            # set_widget_opacity(w, 1.0)
             
    else:
        win.toggleButton_aoc.setPixmap(win.toggle_off_pix)   
        for w in widgets:
            w.setDisabled(True)
            # set_widget_opacity(w, 0.4)
    
    should_enable_dimension_fields = (state == 'ON') and not is_off
    for w in DimensionWise_Enable_Disabled_widgets:
        w.setEnabled(should_enable_dimension_fields)
...
def update_networkdatabase_toggle(win):
    ...
    if state == 'ON' :
        win.toggleButton_networkBasedDatabase.setPixmap(win.toggle_on_pix)
        for w in widgets:
            w.setEnabled(True)
            # set_widget_opacity(w, 1.0)
        ...
        win.toggleButton_networkBasedDatabase.setPixmap(win.toggle_off_pix)
        for w in widgets:
            w.setDisabled(True)
            # set_widget_opacity(w, 0.4)
                  

def update_IOSetting_ReadingFreeze_toggle(win):
    """
    Updates the IoSetting READING_FREEZE toggle image, enables/disables widgets,
    and sets opacity based on the dictionary value in win.valueObj.
    """
    state = win.valueObj.IOSettings_dict.get('READING_FREEZE', {}).get('Enable', '0')
    win.valueObj.activeVariables_dict["BuzzerOnOff"] = state 
    if state == '1':
        win.toggleButton_readingFreeze.setPixmap(win.toggle_on_pix) 
    else:
        win.toggleButton_readingFreeze.setPixmap(win.toggle_off_pix)
...
def update_IOSetting_Relay_toggle(win):
    ...
    if state == '1':
        win.toggelButton_relay.setPixmap(win.toggle_on_pix)
        for w in widgets:
            w.setEnabled(True)
            # # set_widget_opacity(w, 1.0)
             
    else:
        win.toggelButton_relay.setPixmap(win.toggle_off_pix)
        for w in widgets:
            w.setDisabled(True)
            # # set_widget_opacity(w, 0.4)
...
def update_IOSetting_CycleStopTimer_toggle(win):
    ...
    if state == '1':
        win.toggelButton_cycleStopTimer.setPixmap(win.toggle_on_pix)
        for w in widgets:
            w.setEnabled(True)
            # # set_widget_opacity(w, 1.0)
             
    else:
        win.toggelButton_cycleStopTimer.setPixmap(win.toggle_off_pix)
        for w in widgets:
            w.setDisabled(True)
            # # set_widget_opacity(w, 0.4)
...
def update_IOSetting_AutosaveReading_toggle(win):
    ...
    if state == '1':
        win.toggelButton_autoSaveReading.setPixmap(win.toggle_on_pix) 
    else:
        win.toggelButton_autoSaveReading.setPixmap(win.toggle_off_pix)
...
def update_IOSetting_PartTraceability_toggle(win):
    ...
    if state == '1':
        win.toggelButton_partTraceability.setPixmap(win.toggle_on_pix)
        for w in widgets:
            w.setEnabled(True)
            # set_widget_opacity(w, 1.0)
        ...
        win.toggelButton_partTraceability.setPixmap(win.toggle_off_pix)
        for w in widgets:
            w.setDisabled(True)
            # set_widget_opacity(w, 0.4)
...
def update_IOSetting_TimetoMasterSet_toggle(win):
    ...
    if state == '1':
        win.toggleButtontimeToSetMaster.setPixmap(win.toggle_on_pix)
        for w in widgets:
            w.setEnabled(True)
            # set_widget_opacity(w, 1.0)
            
    else:
        win.toggleButtontimeToSetMaster.setPixmap(win.toggle_off_pix)
        for w in widgets:
            w.setDisabled(True)
            # set_widget_opacity(w, 0.4)
...
def update_IOSetting_MasterGrouping_toggle(win):
    ...
    if state == '1':
        win.toggleButton_masterGrouping.setPixmap(win.toggle_on_pix)    
    else:
        win.toggleButton_masterGrouping.setPixmap(win.toggle_off_pix)

def update_IOSetting_SaveMasterCalibration_toggle(win):
    """
    Updates the IoSetting MasterGrouping toggle image, enables/disables widgets,
    and sets opacity based on the dictionary value in win.valueObj.
    """
    state = win.valueObj.IOSettings_dict.get('SAVE_MASTER_CALIBRATION', {}).get('Enable', 'OFF')     
    win.valueObj.activeVariables_dict["SaveMasterCalibrationOnOff"] = state
    
    if state == 'ON':
        win.toggleButtonSaveMasterCalibration.setPixmap(win.toggle_on_pix)    
    else:
        win.toggleButtonSaveMasterCalibration.setPixmap(win.toggle_off_pix)
