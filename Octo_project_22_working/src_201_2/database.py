...
class DatabaseAgent:
    def __init__(self):
        ...
        engine = create_engine("sqlite:////home/torizon/app/data/precigo.db")
        ...
        try:
            with self.SessionLocal() as session:
                ...
                if cnc_id is None:
                    ...
                    existing = (
                        session.query(CNCMaster)
                        .filter_by(CNCName=cnc_name)
                        .first()
                    )
                ...
                # ==========================================
                    ...
                    # Check if ANOTHER CNC has same name
                    duplicate = (
                        session.query(CNCMaster)
                        .filter(
                            CNCMaster.CNCName == cnc_name,
                            CNCMaster.id != cnc_id
                        )
                        .first()
                    )
                    ...
                    # Update SAME row
                    cnc.CNCName = cnc_name
                    cnc.IPAddress = ip
                    cnc.PortNumber = port
                    cnc.CNCSelection = selection
                    cnc.ControllerName = controller
                    cnc.Trial_mode = trial_mode
            ...
            print(f"Error in upsert_cnc_list: {e}")
            return "error"
    ...
    def update_io_settings(
            ...
            - Example: READING_FREEZE, RELAY, AUTO_SAVE_READING.
            """
            with self.SessionLocal() as session:
                try:
                    ...
                    upsert("READING_FREEZE", READING_FREEZE)
                    upsert("RELAY", RELAY)
    ...
    #                 return False
    def upsert_user_management(self, user_id, username, password, access_type):
        try:
            with self.SessionLocal() as session:
                ...
                # MODIFY MODE
                    ...
                    first_user = (
                        session.query(UserManagement)
                        .order_by(UserManagement.id)
                        .first()
                    )
                    if (
                        first_user
                        and user.id == first_user.id
    ...
    def upsert_aoc_based_settings(
            ...
            direction: str = None,
            turret_no: int = None,
            work_in_process: int = None,
            skipoffsetCount:int = None
        ):
            """
            ...
            with self.SessionLocal() as session:
                try:
                    ...
                    if existing:
                        ...
                        existing.WorkInProcess = work_in_process
                        existing.SkipOffsetCount = skipoffsetCount
                    else:
                        session.add(
                            AOCBasedSettings(
                                ProgramId=program_id,
            ...
            try:
                ...
                if not self.IOSettings_dict:
                    self.IOSettings_dict = {
                        ...
                        'READING_FREEZE': {'Enable': '0', 'Value': None},
            ...
            try:
                ...
                for aoc in aocs:
                    ...
                    program_dict[dim]["AOCBasedSettings"] = {
                        ...
                        "WorkInProcess": aoc.WorkInProcess,
                        "SkipOffsetCount": aoc.SkipOffsetCount
                    }
