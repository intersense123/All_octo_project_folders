...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):#, gif_labels: dict):
        try:
            ...
            # Set default program ID to 1
            # self.comboBox_programId.setCurrentText("1")
            #QTimer.singleShot(0, self.labels_hide_from_ProgramSettings)
    ...
    def update_ui_after_programSettings_saved(self):
        current_program_id = self.comboBox_programIdSettings.currentText()
        current_dimension = self.comboBox_toselectProbe.currentText()
        self.valueObj.activeVariables_dict["ActiveProgramId"] = current_program_id
        self.init_progId_and_dimension()

        self.comboBox_programIdSettings.setCurrentText(self.valueObj.activeVariables_dict["ActiveProgramId"])
        self.comboBox_programIdSetting.setCurrentText(self.valueObj.activeVariables_dict["ActiveProgramId"])
        self.comboBox_toselectProbe.setCurrentText(current_dimension)
    ...
    def init_progId_and_dimension(self):
        ...
        self.comboBox_programIdSetting.addItems([str(pid) for pid in proIds])

        if proIds:
            next_id = max(proIds) + 1
            program_ids = proIds
            program_ids.append(next_id)
        else:
            program_ids = [1]  # if table empty

        self.comboBox_programIdSettings.clear()
        self.comboBox_programIdSettings.addItems([str(pid) for pid in program_ids])

        active_id = self.valueObj.activeVariables_dict.get("ActiveProgramId")
        
        self.comboBox_programIdSetting.setCurrentText(str(active_id))
        self.comboBox_programIdSettings.setCurrentText(str(active_id))

        # dimensions = [key for key in self.valueObj.ProgramSettings_dict.keys() if key != "ProgramSpecificSettings"]
        dimensions = [key for key in (self.valueObj.ProgramSettings_dict or {}).keys() if key != "ProgramSpecificSettings"]

        if not dimensions:
            dimensions = ['D1']
        else:
            # Get the last dimension number
            last_num = int(dimensions[-1][1:])  # 'D2' -> 2
            # if last_num >= 8:
            #     pass  # reached max limit, do nothing

            # Add next dimension
            dimensions.append(f'D{last_num + 1}')
            # return dim_list

        self.comboBox_toselectProbe.clear()
        self.comboBox_toselectProbe.addItems([str(did) for did in dimensions])
        
    # Load data with ProgramId to dict ### Handling Outer ProgramId Combobox
    def load_as_per_progId(self):
        try:
            default_programId = self.valueObj.activeVariables_dict.get('ActiveProgramId',1)
            
            self.valueObj.ProgramSettings_dict = self.databaseObj.load_data_to_ProgramSettings_dict(default_programId)
            print(self.valueObj.ProgramSettings_dict,'hahaha')
            self.load_ProgramSettings_to_ui()
            self.load_as_per_dimension('D1')
            # self.load_ProbeBasedSettings_to_ui(self.comboBox_toselectProbe.currentText())
            # self.load_AOCBasedSettings_to_ui(self.comboBox_toselectProbe.currentText())
            ...
            print(f"Error in load_as_per_progId: {e}")
            msg = CustomMessageBox("Failed to load program settings. Please check the database and try again.", "error")
            msg.exec_()
    ...
    def handle_set_datetime(self):
        ...
        try:
            if self.utils.is_raspberry_pi():
                self.set_system_time_raspberyPi() # Assuming this method exists and handles its own exceptions
            elif self.utils.is_imx7():
                self.set_system_time_imx7() # Assuming this method exists and handles its own exceptions
            else:
    ...
    def load_databases(self):
        ...
        try:
            self.valueObj.ProgramSettings_dict = self.databaseObj.load_data_to_ProgramSettings_dict(1)
        except Exception as e:
    ...
    def labels_hide_from_ProgramSettings(self):
        try:
            self.button_backSettings.show()
            self.button_fowardSettings.show()
            total_pages = self.stackedWidget_2.count()
            current_index = self.stackedWidget_2.currentIndex()

            # Update label text → e.g. "Page 1 of 4"
            self.label_pageNo.setText(f"Page {current_index + 1} of {total_pages}")
            self.label_pageNo.setAlignment(QtCore.Qt.AlignCenter)

            # Show both navigation buttons first
            self.button_backSettings.show()
            self.button_fowardSettings.show()
            
            if self.stackedWidget_2.currentIndex() == 0:
                self.button_backSettings.hide()
            elif self.stackedWidget_2.currentIndex() == 3:
                self.button_fowardSettings.hide()
        except Exception as e:
            print(f"Error in hideing labels : {e}")
    ...
    def load_ProbeBasedSettings_to_ui(self,dimension):
        try:
            ...
            if not hasattr(self.valueObj, 'ProgramSettings_dict') or not self.valueObj.ProgramSettings_dict:
                # This might need to load a specific program_id, assuming a default or active one.
            ...
            #print(self.valueObj.ProgramSettings_dict,"probebased database.........................")
            if not dim_data:
            ...
            self.lineEdit_masterLower.setText(str(probe_settings.get("MasterLower", "0.0")))
            self.lineEdit_master.setText(str(probe_settings.get("Master", "0.0")))
            self.lineEdit_masterHigher.setText(str(probe_settings.get("MasterHigher", "0.0")))
            
            self.lineEdit_upperOffcetLimit.setText(str(probe_settings.get("UpperOffsetLimit", "0.0")))
            self.lineEdit_usl.setText(str(probe_settings.get("UpperSpecificationLimit", "0.0")))
            self.lineEdit_ucl.setText(str(probe_settings.get("UpperControlLimit", "0.0")))
            self.lineEdit_nominalValue.setText(str(probe_settings.get("NominalValue", "0.0")))
            self.lineEdit_lcl.setText(str(probe_settings.get("LowerControlLimit", "0.0")))
            self.lineEdit_lsl.setText(str(probe_settings.get("LowerSpecificationLimit", "0.0")))
            self.lineEdit_lowerOffsetLimit.setText(str(probe_settings.get("LowerOffsetLimit", "0.0")))
            ...
            self.lineEdit_caseT.setText(str(probe_settings.get("CaseT", "0.0")))
            self.lineEdit_probeSensitivity.setText(str(probe_settings.get("ProbeSensitivity", "0")))
            self.lineEdit_airSensitivityQuotient.setText(str(probe_settings.get("AirSensitivityQuotient", "")))
        except Exception as e:
            print(f"Error loading ProbeBasedSettings to UI for dimension {dimension}: {e}")
    ...
    def load_AOCBasedSettings_to_ui(self,dimension):
        try:
            ...
            self.lineEdit_offsetNo.setText(str(aoc_settings.get("OffsetNo", "0")))
            ...
            self.lineEdit_turretNo.setText(str(aoc_settings.get("TurretNo", "0")))
            self.lineEdit_bufferPartNo.setText(str(aoc_settings.get("BufferPartNo", "0")))
